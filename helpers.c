...
gboolean app_id_to_triplet(const gchar* app_id, gchar** package, gchar** application, gchar** version)
{
    /* 'Parse' the App ID */
...
gchar* manifest_to_desktop(const gchar* app_dir, const gchar* app_id)
{
    ...
    {
        g_warning("Manifest '%s' version '%s' doesn't match AppID version '%s'", package,
                  json_object_get_string_member(manifest, "version"), version);
        goto manifest_out;
    }
...
static inline void file_list_handling(GArray* outarray, gchar** list, gchar* (*dup_func)(const gchar* in))
{
...
static void desktop_exec_segment_parse(GArray* finalarray, const gchar* execsegment, gchar** uri_list)
{
    /* No NULL strings */
 ...
 */
void set_confined_envvars(EnvHandle* handle, const gchar* package, const gchar* app_dir)
{
...
static void unity_signal_cb(GDBusConnection* con,
                            const gchar* sender,
                            const gchar* path,
...
handshake_t* starting_handshake_start(const gchar* app_id, const gchar* instance_id, int timeout_s)
{
    GError* error = NULL;
    ...
    handshake->signal_subscribe = g_dbus_connection_signal_subscribe(handshake->con, NULL,            /* sender */
                                                                     "com.canonical.UbuntuAppLaunch", /* interface */
                                                                     "UnityStartingSignal",           /* signal */
                                                                     "/",                             /* path */
                                                                     app_id,                          /* arg0 */
                                                                     G_DBUS_SIGNAL_FLAGS_NONE, unity_signal_cb,
                                                                     handshake->mainloop, NULL); /* user data destroy */
    ...
    g_dbus_connection_emit_signal(handshake->con, NULL,            /* destination */
                                  ...
                                  g_variant_new("(ss)", app_id, instance_id), &error);
...
void env_handle_add(EnvHandle* handle, const gchar* variable, const gchar* value)
{
...
void env_handle_finish(EnvHandle* handle)
{
    ...
    GError* error = NULL;
    GVariant* reply = g_dbus_connection_call_sync(bus, DBUS_SERVICE_UPSTART, DBUS_PATH_UPSTART, DBUS_INTERFACE_UPSTART,

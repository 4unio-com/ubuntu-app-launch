...
void
set_upstart_variable (const gchar * variable, const gchar * value, GAsyncReadyCallback callback, gpointer userdata)
{
...
set_confined_envvars (const gchar * package, const gchar * app_dir)
{
	...
	set_upstart_variable("UBUNTU_APPLICATION_ISOLATION", "1", NULL, NULL);
	...
	set_upstart_variable("XDG_CACHE_HOME", g_get_user_cache_dir(), NULL, NULL);

	g_debug("Setting 'XDG_CONFIG_HOME' using g_get_user_config_dir()");
	set_upstart_variable("XDG_CONFIG_HOME", g_get_user_config_dir(), NULL, NULL);

	g_debug("Setting 'XDG_DATA_HOME' using g_get_user_data_dir()");
	set_upstart_variable("XDG_DATA_HOME", g_get_user_data_dir(), NULL, NULL);

	g_debug("Setting 'XDG_RUNTIME_DIR' using g_get_user_runtime_dir()");
	set_upstart_variable("XDG_RUNTIME_DIR", g_get_user_runtime_dir(), NULL, NULL);

	/* Add the application's dir to the list of sources for data */
	gchar * datadirs = g_strjoin(":", app_dir, g_getenv("XDG_DATA_DIRS"), NULL);
	set_upstart_variable("XDG_DATA_DIRS", datadirs, NULL, NULL);
	g_free(datadirs);
	...
	set_upstart_variable("TMPDIR", tmpdir, NULL, NULL);
	...
	set_upstart_variable("__GL_SHADER_DISK_CACHE_PATH", nv_shader_cachedir, NULL, NULL);

...
gboolean app_id_to_triplet(const gchar *app_id, gchar **package,
...
gchar *manifest_to_desktop(const gchar *app_dir, const gchar *app_id) {
  gchar *package = NULL;
  ...
  if (g_strcmp0(json_object_get_string_member(manifest, "version"), version) !=
      0) {
    g_warning("Manifest '%s' version '%s' doesn't match AppID version '%s'",
              package, json_object_get_string_member(manifest, "version"),
              version);
    goto manifest_out;
  }
  ...
  if (appsobj == NULL) {
    g_warning("Manifest '%s' has an hooks section that is not a JSON object",
              package);
    goto manifest_out;
  }

  if (!json_object_has_member(appsobj, application)) {
    g_warning("Manifest '%s' doesn't have the application '%s' defined",
              package, application);
    goto manifest_out;
  }
  ...
  if (appobj == NULL) {
    g_warning(
        "Manifest '%s' has a definition for application '%s' that is not an "
        "object",
        package, application);
    goto manifest_out;
  }
...
manifest_out:
  ...
  if (manifest != NULL) json_object_unref(manifest);
  g_clear_object(&user);
  g_clear_object(&db);
  g_free(package);
  g_free(application);
...
gchar *desktop_to_exec(GKeyFile *desktop_file, const gchar *from) {
  GError *error = NULL;
  ...
  gchar *type =
      g_key_file_get_string(desktop_file, "Desktop Entry", "Type", &error);
  if (error != NULL) {
  ...
  if (g_strcmp0(type, "Application") != 0) {
    g_warning("Desktop file '%s' has a type of '%s' instead of 'Application'",
              from, type);
    g_free(type);
    return NULL;
  }
  ...
  if (g_key_file_has_key(desktop_file, "Desktop Entry", "NoDisplay", NULL)) {
    gboolean nodisplay = g_key_file_get_boolean(desktop_file, "Desktop Entry",
                                                "NoDisplay", NULL);
    if (nodisplay) {
  ...
  if (g_key_file_has_key(desktop_file, "Desktop Entry", "Hidden", NULL)) {
    gboolean hidden =
        g_key_file_get_boolean(desktop_file, "Desktop Entry", "Hidden", NULL);
    if (hidden) {
  ...
  if (g_key_file_has_key(desktop_file, "Desktop Entry", "Terminal", NULL)) {
    ...
    if (terminal) {
      g_warning("Desktop file '%s' is set to run in a terminal, not copying",
                from);
      return NULL;
    }
  }
  ...
  gchar *exec =
      g_key_file_get_string(desktop_file, "Desktop Entry", "Exec", NULL);
  return exec;
}

/* Convert a URI into a file */
static gchar *uri2file(const gchar *uri) {
  GError *error = NULL;
...
static inline void file_list_handling(GArray *outarray, gchar **list,
                                      gchar *(*dup_func)(const gchar *in)) {
...
static void desktop_exec_segment_parse(GArray *finalarray,
                                       const gchar *execsegment,
                                       gchar **uri_list) {
  /* No NULL strings */
  if (execsegment == NULL || execsegment[0] == '\0') return;
    ...
    switch (execsplit[i][0]) {
      ...
      case 'f':
        if (uri_list != NULL && uri_list[0] != NULL) {
          if (single_file == NULL) single_file = uri2file(uri_list[0]);
      ...
      case 'F':
        g_warning(
            "Exec line segment has a '%%F' that isn't its own argument '%s', "
            "ignoring.",
            execsegment);
        g_array_append_val(outarray, skipchar);
        break;
      ...
      case 'U':
        g_warning(
            "Exec line segment has a '%%U' that isn't its own argument '%s', "
            "ignoring.",
            execsegment);
        g_array_append_val(outarray, skipchar);
        break;
      case 'u':
        ...
        break;
      default:
        g_warning("Desktop Exec line code '%%%c' unknown, skipping.",
                  execsplit[i][0]);
        g_array_append_val(outarray, skipchar);
        break;
    }
...
/* Take a full exec line, split it out, parse the segments and return
   it to the caller */
GArray *desktop_exec_parse(const gchar *execline, const gchar *urilist) {
  GError *error = NULL;
 ...
 */
void set_confined_envvars(EnvHandle *handle, const gchar *package,
                          const gchar *app_dir) {
  ...
  gchar *tmpdir =
      g_strdup_printf("%s/confined/%s", g_get_user_runtime_dir(), package);
  ...
  gchar *nv_shader_cachedir =
      g_strdup_printf("%s/%s", g_get_user_cache_dir(), package);
...
static void unity_signal_cb(GDBusConnection *con, const gchar *sender,
...
static gboolean unity_too_slow_cb(gpointer user_data) {
...
handshake_t *starting_handshake_start(const gchar *app_id) {
  ...
  handshake->signal_subscribe = g_dbus_connection_signal_subscribe(
      handshake->con, NULL,            /* sender */
      "com.canonical.UbuntuAppLaunch", /* interface */
      "UnityStartingSignal",           /* signal */
      "/",                             /* path */
      app_id,                          /* arg0 */
      G_DBUS_SIGNAL_FLAGS_NONE, unity_signal_cb, handshake->mainloop,
      NULL); /* user data destroy */

  /* Send unfreeze to to Unity */
  g_dbus_connection_emit_signal(handshake->con, NULL, /* destination */
                                "/",                  /* path */
                                "com.canonical.UbuntuAppLaunch", /* interface */
                                "UnityStartingBroadcast",        /* signal */
                                g_variant_new("(s)", app_id), &error);
...
void starting_handshake_wait(handshake_t *handshake) {
  ...
  if (handshake->timeout != 0) g_source_remove(handshake->timeout);
  g_main_loop_unref(handshake->mainloop);
  g_dbus_connection_signal_unsubscribe(handshake->con,
                                       handshake->signal_subscribe);
...
void env_handle_add(EnvHandle *handle, const gchar *variable,
                    const gchar *value) {
...
void env_handle_finish(EnvHandle *handle) {
  ...
  g_variant_builder_add_value(&builder,
                              g_variant_builder_end((GVariantBuilder *)handle));
  ...
  GVariant *reply = g_dbus_connection_call_sync(
      bus, DBUS_SERVICE_UPSTART, DBUS_PATH_UPSTART, DBUS_INTERFACE_UPSTART,
      "SetEnvList", g_variant_builder_end(&builder), NULL, /* reply */

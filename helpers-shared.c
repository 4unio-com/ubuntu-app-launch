...
static void cgroup_manager_connection_core_cb(GDBusConnection* (*finish_func)(GAsyncResult* res, GError** error),
                                              GAsyncResult* res,
...
static void cgroup_manager_connection_bus_cb(GObject* obj, GAsyncResult* res, gpointer data)
{
    cgroup_manager_connection_core_cb(g_bus_get_finish, res, (cgm_connection_t*)data);
}

static void cgroup_manager_connection_addr_cb(GObject* obj, GAsyncResult* res, gpointer data)
{
...
GDBusConnection* cgroup_manager_connection(void)
{
    ...
    cgm_connection_t connection = {.loop = g_main_loop_new(context, FALSE), .con = NULL, .cancel = g_cancellable_new()};
    ...
    if (use_session_bus)
    {
        ...
        g_bus_get(G_BUS_TYPE_SESSION, connection.cancel, cgroup_manager_connection_bus_cb, &connection);
    }
    else
    {
        g_dbus_connection_new_for_address(CGMANAGER_DBUS_PATH, G_DBUS_CONNECTION_FLAGS_AUTHENTICATION_CLIENT,
                                          NULL,              /* Auth Observer */
    ...
    {
        g_object_set_qdata(G_OBJECT(connection.con), cgmanager_context_quark(), context);
    }
    else
    {
...
GList* pids_from_cgroup(GDBusConnection* cgmanager, const gchar* jobname, const gchar* instancename)
{
    GError* error = NULL;
    ...
    GVariant* vtpids =
        g_dbus_connection_call_sync(cgmanager, name, /* bus name for direct connection is NULL */
                                    "/org/linuxcontainers/cgmanager", "org.linuxcontainers.cgmanager0_0",
                                    "GetTasksRecursive", g_variant_new("(ss)", "freezer", groupname ? groupname : ""),

...
gboolean verify_keyfile(GKeyFile *inkeyfile, const gchar *desktop) {
  ...
  if (!g_key_file_has_group(inkeyfile, "Desktop Entry")) {
    g_warning("Desktop file '%s' is missing the 'Desktop Entry' group",
              desktop);
    return FALSE;
  }
...
static GKeyFile *try_dir(const char *dir, const gchar *desktop) {
  ...
  gboolean loaded =
      g_key_file_load_from_file(keyfile, fullpath, G_KEY_FILE_NONE, NULL);
...
GKeyFile *keyfile_for_appid(const gchar *appid, gchar **desktopfile) {
  ...
  if (keyfile != NULL && desktopfile != NULL && *desktopfile == NULL) {
    *desktopfile =
        g_build_filename(g_get_user_data_dir(), "applications", desktop, NULL);
  }
  ...
  for (i = 0; data_dirs[i] != NULL && keyfile == NULL; i++) {
    ...
    if (keyfile != NULL && desktopfile != NULL && *desktopfile == NULL) {
      *desktopfile =
          g_build_filename(data_dirs[i], "applications", desktop, NULL);
    }
  }
...
static gboolean cgroup_manager_connection_timeout_cb(gpointer data) {
...
static void cgroup_manager_connection_core_cb(
    GDBusConnection *(*finish_func)(GAsyncResult *res, GError **error),
    GAsyncResult *res, cgm_connection_t *connection) {
  GError *error = NULL;
...
static void cgroup_manager_connection_bus_cb(GObject *obj, GAsyncResult *res,
                                             gpointer data) {
  cgroup_manager_connection_core_cb(g_bus_get_finish, res,
                                    (cgm_connection_t *)data);
}

static void cgroup_manager_connection_addr_cb(GObject *obj, GAsyncResult *res,
                                              gpointer data) {
  cgroup_manager_connection_core_cb(g_dbus_connection_new_for_address_finish,
                                    res, (cgm_connection_t *)data);
}
...
GDBusConnection *cgroup_manager_connection(void) {
  gboolean use_session_bus =
      g_getenv("UBUNTU_APP_LAUNCH_CG_MANAGER_SESSION_BUS") != NULL;
  GMainContext *context = g_main_context_new();
  g_main_context_push_thread_default(context);

  cgm_connection_t connection = {.loop = g_main_loop_new(context, FALSE),
                                 .con = NULL,
                                 .cancel = g_cancellable_new()};

  GSource *timesrc = g_timeout_source_new_seconds(1);
  g_source_set_callback(timesrc, cgroup_manager_connection_timeout_cb,
                        &connection, NULL);
  ...
  if (use_session_bus) {
    ...
    g_bus_get(G_BUS_TYPE_SESSION, connection.cancel,
              cgroup_manager_connection_bus_cb, &connection);
  } else {
    g_dbus_connection_new_for_address(
        CGMANAGER_DBUS_PATH, G_DBUS_CONNECTION_FLAGS_AUTHENTICATION_CLIENT,
        NULL,              /* Auth Observer */
        connection.cancel, /* Cancellable */
        cgroup_manager_connection_addr_cb, &connection);
  }
  ...
  if (!use_session_bus && connection.con != NULL) {
    g_object_set_qdata(G_OBJECT(connection.con), cgmanager_context_quark(),
                       context);
  } else {
...
void cgroup_manager_unref(GDBusConnection *cgmanager) {
  if (cgmanager == NULL) return;

  GMainContext *creationcontext =
      g_object_get_qdata(G_OBJECT(cgmanager), cgmanager_context_quark());
...
GList *pids_from_cgroup(GDBusConnection *cgmanager, const gchar *jobname,
  ...
  GVariant *vtpids = g_dbus_connection_call_sync(
      cgmanager, name, /* bus name for direct connection is NULL */
      "/org/linuxcontainers/cgmanager", "org.linuxcontainers.cgmanager0_0",
      "GetTasksRecursive",
      g_variant_new("(ss)", "freezer", groupname ? groupname : ""),
      G_VARIANT_TYPE("(ai)"), G_DBUS_CALL_FLAGS_NONE, -1, /* default timeout */
      NULL,                                               /* cancellable */
      &error);

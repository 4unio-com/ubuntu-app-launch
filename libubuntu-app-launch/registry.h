...
/** The application registry provides a central source for finding information
    ...
    This class also holds onto shared resources for Ubuntu App Launch objects
   and
    functions. Generally speaking, there should only be one of them in the
    process. There are singleton functions, getDefault() and clearDefault(),
   which
...
class Registry {
 public:
  ...
  enum class FailureType {
    ...
    START_FAILURE, /**< Something in the configuration of the application made
                      it impossible to start the
                      application */
  };
  ...
  */
  static std::list<std::shared_ptr<Application>> runningApps(
  ...
  /** List all of the applications that are currently installed on the system.
      ...
      \param registry Shared registry for the tracking
  */
  static std::list<std::shared_ptr<Application>> installedApps(
      std::shared_ptr<Registry> registry = getDefault());
  ...
  /** Get a list of all the helpers for a given helper type
      ...
      \param registry Shared registry for the tracking
  */
  static std::list<std::shared_ptr<Helper>> runningHelpers(
      Helper::Type type, std::shared_ptr<Registry> registry = getDefault());

...
std::pair<std::string, std::shared_ptr<GKeyFile>> keyfileForApp(
    const AppID::AppName& name);
...
Legacy::Legacy(const AppID::AppName& appname, const std::string& basedir,
               const std::shared_ptr<GKeyFile>& keyfile,
               const std::shared_ptr<Registry>& registry)
    : Base(registry), _appname(appname), _basedir(basedir), _keyfile(keyfile) {
  if (!_keyfile)
    throw std::runtime_error{"Unable to find keyfile for legacy application: " +
                             appname.value()};
}

Legacy::Legacy(const AppID::AppName& appname,
               const std::shared_ptr<Registry>& registry)
    : Base(registry), _appname(appname) {
...
std::pair<std::string, std::shared_ptr<GKeyFile>> keyfileForApp(
  ...
  auto keyfilecheck =
      [desktopName](const std::string& dir) -> std::shared_ptr<GKeyFile> {
    auto fullname = g_build_filename(dir.c_str(), "applications",
                                     desktopName.c_str(), nullptr);
    ...
    if (error != nullptr) {
      g_debug("Unable to load keyfile '%s' becuase: %s", desktopName.c_str(),
              error->message);
      g_error_free(error);
      return {};
    }
...
std::list<std::shared_ptr<Application>> Legacy::list(
  ...
  for (GList* item = head; item != nullptr; item = g_list_next(item)) {
    ...
    try {
      auto app =
          std::make_shared<Legacy>(AppID::AppName::from_raw(appname), registry);
      list.push_back(app);
    } catch (std::runtime_error& e) {
      g_debug("Unable to create application for legacy appname '%s': %s",

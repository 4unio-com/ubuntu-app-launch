...
ContextThread::ContextThread(std::function<void()> beforeLoop,
                             std::function<void()> afterLoop) {
  _cancel = std::shared_ptr<GCancellable>(g_cancellable_new(),
                                          [](GCancellable* cancel) {
                                            if (cancel != nullptr) {
  ...
  std::promise<
      std::pair<std::shared_ptr<GMainContext>, std::shared_ptr<GMainLoop>>>
      context_promise;
  ...
  _thread = std::thread([&context_promise, &beforeLoop, afterLoop, this]() {
    ...
    auto context = std::shared_ptr<GMainContext>(
        g_main_context_new(), [](GMainContext* context) {
          g_clear_pointer(&context, g_main_context_unref);
        });
    auto loop = std::shared_ptr<GMainLoop>(
        g_main_loop_new(context.get(), FALSE),
...
ContextThread::~ContextThread() { quit(); }

void ContextThread::quit() {
  g_cancellable_cancel(
      _cancel.get()); /* Force the cancellation on ongoing tasks */
  if (_loop) {
...
void ContextThread::simpleSource(std::function<GSource*()> srcBuilder,
                                 std::function<void()> work) {
  if (isCancelled()) {
    throw std::runtime_error(
        "Trying to execute work on a GLib thread that is shutting down.");
  }
...
void ContextThread::timeout(const std::chrono::milliseconds& length,
                            std::function<void()> work) {
  simpleSource([length]() { return g_timeout_source_new(length.count()); },
               work);
}
...
void ContextThread::timeoutSeconds(const std::chrono::seconds& length,
                                   std::function<void()> work) {
  simpleSource(
      [length]() { return g_timeout_source_new_seconds(length.count()); },
      work);
}

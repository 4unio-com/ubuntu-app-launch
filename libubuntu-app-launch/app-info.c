...
JsonObject *get_manifest(const gchar *pkg, gchar **pkgpath) {
  ...
  if (error != NULL) {
    g_warning("Unable to get manifest for '%s' package: %s", pkg,
              error->message);
    g_error_free(error);
    g_object_unref(user);
    return NULL;
  ...
  if (pkgpath != NULL) {
    *pkgpath = click_user_get_path(user, pkg, &error);
    if (error != NULL) {
      g_warning("Unable to get the Click package directory for %s: %s", pkg,
                error->message);
      g_error_free(error);
      g_object_unref(user);
      return NULL;
...
enum _app_name_t { APP_NAME_ONLY, APP_NAME_FIRST, APP_NAME_LAST };
typedef enum _app_name_t app_name_t;

/* Figure out the app name if it's one of the keywords */
static const gchar *manifest_app_name(JsonObject **manifest, const gchar *pkg,
                                      const gchar *original_app) {
...
static const gchar *manifest_version(JsonObject **manifest, const gchar *pkg,
                                     const gchar *original_ver) {
  if (original_ver != NULL &&
      g_strcmp0(original_ver, "current-user-version") != 0) {
    return original_ver;
  } else {
    if (*manifest == NULL) {
      *manifest = get_manifest(pkg, NULL);
...
gchar *click_triplet_to_app_id(const gchar *pkg, const gchar *app,
                               const gchar *ver) {
  ...
  if (manifest) json_object_unref(manifest);
...
gchar *libertine_triplet_to_app_id(const gchar *pkg, const gchar *app,
                                   const gchar *ver) {
  if (app == NULL) {
    return NULL;
  }
...
/* Look to see if the app id results in a desktop file, if so, fill in the
 * params */
static gboolean evaluate_dir(const gchar *dir, const gchar *desktop,
                             gchar **appdir, gchar **appdesktop) {
...
gboolean app_info_legacy(const gchar *appid, gchar **appdir,
                         gchar **appdesktop) {
...
gboolean app_info_libertine(const gchar *appid, gchar **appdir,
                            gchar **appdesktop) {
  char *container = NULL;
  char *app = NULL;
  ...
  gchar *desktopdir =
      g_build_filename(g_get_user_cache_dir(), "libertine-container", container,
                       "rootfs", "usr", "share", NULL);
  gchar *desktopfile =
      g_build_filename(desktopdir, "applications", desktopname, NULL);
  ...
  if (!g_file_test(desktopfile, G_FILE_TEST_EXISTS)) {
    ...
    desktopdir =
        g_build_filename(g_get_user_data_dir(), "libertine-container",
                         "user-data", container, ".local", "share", NULL);
    desktopfile =
        g_build_filename(desktopdir, "applications", desktopname, NULL);
...
gboolean app_info_click(const gchar *appid, gchar **appdir,
                        gchar **appdesktop) {
  gchar *package = NULL;

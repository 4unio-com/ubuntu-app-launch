...
static void apps_for_job(GDBusConnection *con, const gchar *name, GArray *apps,
...
static gchar *app_uris_string(const gchar *const *uris) {
  guint i = 0;
...
static void application_start_cb(GObject *obj, GAsyncResult *res,
                                 gpointer user_data) {
  ...
  if (error != NULL) {
    if (g_dbus_error_is_remote_error(error)) {
      ...
      if (g_strcmp0(remote_error,
                    "com.ubuntu.Upstart0_6.Error.AlreadyStarted") == 0) {
        second_exec(data->appid, data->uris);
      }

      g_free(remote_error);
    } else {
      g_warning("Unable to emit event to start application: %s",
                error->message);
    }
    g_error_free(error);
  }
...
static const gchar *get_jobpath(GDBusConnection *con, const gchar *jobname) {
  gchar *cachepath =
      g_strdup_printf("ubuntu-app-lauch-job-path-cache-%s", jobname);
  ...
  GError *error = NULL;
  GVariant *job_path_variant = g_dbus_connection_call_sync(
      con, DBUS_SERVICE_UPSTART, DBUS_PATH_UPSTART, DBUS_INTERFACE_UPSTART,
      "GetJobByName", g_variant_new("(s)", jobname), G_VARIANT_TYPE("(o)"),
...
static gboolean legacy_single_instance(const gchar *appid) {
  ...
  if (g_key_file_has_key(keyfile, "Desktop Entry", "X-Ubuntu-Single-Instance",
                         NULL)) {
    GError *error = NULL;

    singleinstance = g_key_file_get_boolean(keyfile, "Desktop Entry",
                                            "X-Ubuntu-Single-Instance", &error);

    if (error != NULL) {
      g_warning("Unable to get single instance key for app '%s': %s", appid,
                error->message);
      g_error_free(error);
  ...
  ual_tracepoint(desktop_single_finished, appid,
                 singleinstance ? "single" : "unmanaged");
...
static gboolean is_click(const gchar *appid) {
  ...
  if (G_LIKELY(link_farm_dir == NULL)) {
    click_link =
        g_build_filename(g_get_home_dir(), ".cache", "ubuntu-app-launch",
                         "desktop", appiddesktop, NULL);
  } else {
...
static gboolean is_libertine(const gchar *appid) {
...
static gboolean start_application_core(const gchar *appid,
  ...
  ual_tracepoint(libual_determine_libertine, appid,
                 libertine ? "container" : "host");
  ...
  g_variant_builder_add_value(
      &builder, g_variant_new_take_string(g_strdup_printf("APP_ID=%s", appid)));
  g_variant_builder_add_value(
      &builder, g_variant_new_take_string(
                    g_strdup_printf("APP_LAUNCHER_PID=%d", getpid())));
  ...
  if (!click) {
    if (libertine || legacy_single_instance(appid)) {
      g_variant_builder_add_value(&builder,
                                  g_variant_new_string("INSTANCE_ID="));
    } else {
      gchar *instanceid =
          g_strdup_printf("INSTANCE_ID=%" G_GUINT64_FORMAT, g_get_real_time());
      g_variant_builder_add_value(&builder,
                                  g_variant_new_take_string(instanceid));
    }
  }

  if (test) {
    g_variant_builder_add_value(&builder,
                                g_variant_new_string("QT_LOAD_TESTABILITY=1"));
  }
  ...
  if (click) {
    ...
    setup_complete =
        desktop_task_setup(con, appid, (EnvHandle *)&builder, libertine);
  }
  ...
  if (setup_complete) {
    ...
    g_dbus_connection_call(con, DBUS_SERVICE_UPSTART, jobpath,
                           DBUS_INTERFACE_UPSTART_JOB, "Start",
                           g_variant_builder_end(&builder), NULL,
                           G_DBUS_CALL_FLAGS_NONE, -1, NULL, /* cancelable */
                           application_start_cb, app_start_data);
...
static void stop_job(GDBusConnection *con, const gchar *jobname,
                     const gchar *appname, const gchar *instanceid) {
  g_debug("Stopping job %s app_id %s instance_id %s", jobname, appname,
          instanceid);

  const gchar *job_path = get_jobpath(con, jobname);
  if (job_path == NULL) return;
  ...
  g_variant_builder_close(&builder);
  g_variant_builder_add_value(&builder,
                              g_variant_new_boolean(FALSE)); /* wait */

  GError *error = NULL;
  GVariant *stop_variant = g_dbus_connection_call_sync(
      con, DBUS_SERVICE_UPSTART, job_path, DBUS_INTERFACE_UPSTART_JOB, "Stop",
      g_variant_builder_end(&builder), NULL, G_DBUS_CALL_FLAGS_NONE,
      -1,   /* timeout: default */
      NULL, /* cancelable */
      &error);

  if (error != NULL) {
    g_warning("Unable to stop job %s app_id %s instance_id %s: %s", jobname,
              appname, instanceid, error->message);
    g_error_free(error);
  }
...
gboolean ubuntu_app_launch_stop_application(const gchar *appid) {
  ...
  if (apps->len > 0) g_array_remove_range(apps, 0, apps->len);
  ...
  gchar *appiddash =
      g_strdup_printf("%s-", appid); /* Probably could go RegEx here,
                                        but let's start with just a
                                        prefix lookup */
  for (i = 0; i < apps->len; i++) {
    const gchar *array_id = g_array_index(apps, const gchar *, i);
...
static gboolean use_oom_helper(GPid pid, const gchar *oomscore) {
  GError *error = NULL;
  const gchar *args[4] = {OOM_HELPER, NULL, /* pid */
                          oomscore, NULL};
  ...
  if (error != NULL) {
    g_warning("Unable to launch OOM helper '" OOM_HELPER "' on PID '%d': %s",
              pid, error->message);
    g_error_free(error);
    return FALSE;
  }
...
static gboolean set_oom_value(GPid pid, const gchar *oomscore) {
  ...
  if (adj == NULL) {
    switch (openerr) {
      ...
      default:
        g_warning("Unable to set OOM value for '%d' to '%s': %s", pid, oomscore,
                  strerror(openerr));
        return FALSE;
    }
  }
  ...
  if (writesize == strlen(oomscore)) return TRUE;
  ...
  if (writeerr != 0)
    ...
    g_debug("Unable to set OOM value for '%d' to '%s': Wrote %d bytes", pid,
            oomscore, (int)writesize);
...
static void notify_signalling(GList *pids, const gchar *appid,
  ...
  if (pids == NULL) {
    ...
    for (i = pids; i != NULL; i = g_list_next(i))
      g_variant_builder_add_value(
          &builder, g_variant_new_uint64(GPOINTER_TO_INT(i->data)));
  ...
  if (error != NULL) {
    g_warning("Unable to emit signal '%s' for appid '%s': %s", signal_name,
              appid, error->message);
    g_error_free(error);
  } else {
...
static gboolean signal_to_cgroup(const gchar *appid, int signal,
                                 const gchar *oomscore,
                                 const gchar *signal_name) {
...
static void zg_insert_complete(GObject *obj, GAsyncResult *res,
                               gpointer user_data) {
  GError *error = NULL;
  GArray *result = NULL;
...
static void report_zg_event(const gchar *appid, const gchar *eventtype) {
  gchar *uri = NULL;
  gchar *pkg = NULL;
  gchar *app = NULL;
...
gboolean ubuntu_app_launch_application_info(const gchar *appid, gchar **appdir,
                                            gchar **appdesktop) {
  if (is_click(appid)) {
...
static GDBusConnection *gdbus_upstart_ref(void) {
...
static void observer_cb(GDBusConnection *conn, const gchar *sender,
                        const gchar *object, const gchar *interface,
...
static gboolean add_app_generic(UbuntuAppLaunchAppObserver observer,
                                gpointer user_data, const gchar *signal,
                                GList **list) {
  ...
  *list = g_list_prepend(*list, observert);
  ...
  observert->sighandle = g_dbus_connection_signal_subscribe(
      conn, NULL,             /* sender */
      DBUS_INTERFACE_UPSTART, /* interface */
      "EventEmitted",         /* signal */
      DBUS_PATH_UPSTART,      /* path */
      signal,                 /* arg0 */
      G_DBUS_SIGNAL_FLAGS_NONE, observer_cb, observert,
      NULL); /* user data destroy */

  return TRUE;
}

gboolean ubuntu_app_launch_observer_add_app_started(
    UbuntuAppLaunchAppObserver observer, gpointer user_data) {
  return add_app_generic(observer, user_data, "started", &started_array);
}

gboolean ubuntu_app_launch_observer_add_app_stop(
    UbuntuAppLaunchAppObserver observer, gpointer user_data) {
...
/* Creates the observer structure and registers for the signal with
   GDBus so that we can get a callback */
static gboolean add_session_generic(UbuntuAppLaunchAppObserver observer,
                                    gpointer user_data, const gchar *signal,
                                    GList **list,
                                    GDBusSignalCallback session_cb) {
  GDBusConnection *conn = g_bus_get_sync(G_BUS_TYPE_SESSION, NULL, NULL);
  ...
  *list = g_list_prepend(*list, observert);
  ...
  observert->sighandle = g_dbus_connection_signal_subscribe(
      ...
      "/",                             /* path */
      NULL,                            /* arg0 */
      G_DBUS_SIGNAL_FLAGS_NONE, session_cb, observert,
      NULL); /* user data destroy */
...
static inline void generic_signal_cb(GDBusConnection *conn, const gchar *sender,
                                     const gchar *object,
...
static void focus_signal_cb(GDBusConnection *conn, const gchar *sender,
                            const gchar *object, const gchar *interface,
                            const gchar *signal, GVariant *params,
                            gpointer user_data) {
...
gboolean ubuntu_app_launch_observer_add_app_focus(
    UbuntuAppLaunchAppObserver observer, gpointer user_data) {
  return add_session_generic(observer, user_data, "UnityFocusRequest",
                             &focus_array, focus_signal_cb);
}

/* Handle the resume signal when it occurs, call the observer, then send a
 * signal back when we're done */
static void resume_signal_cb(GDBusConnection *conn, const gchar *sender,
                             const gchar *object, const gchar *interface,
                             const gchar *signal, GVariant *params,
                             gpointer user_data) {
...
gboolean ubuntu_app_launch_observer_add_app_resume(
    UbuntuAppLaunchAppObserver observer, gpointer user_data) {
  return add_session_generic(observer, user_data, "UnityResumeRequest",
                             &resume_array, resume_signal_cb);
}

/* Handle the starting signal when it occurs, call the observer, then send a
 * signal back when we're done */
static void starting_signal_cb(GDBusConnection *conn, const gchar *sender,
                               const gchar *object, const gchar *interface,
                               const gchar *signal, GVariant *params,
                               gpointer user_data) {
...
gboolean ubuntu_app_launch_observer_add_app_starting(
    UbuntuAppLaunchAppObserver observer, gpointer user_data) {
  return add_session_generic(observer, user_data, "UnityStartingBroadcast",
                             &starting_array, starting_signal_cb);
}

/* Handle the failed signal when it occurs, call the observer */
static void failed_signal_cb(GDBusConnection *conn, const gchar *sender,
                             const gchar *object, const gchar *interface,
                             const gchar *signal, GVariant *params,
                             gpointer user_data) {
  ...
  if (observer->func != NULL) {
    ...
    if (g_strcmp0("crash", typestr) == 0) {
      ...
      g_warning("Application failure type '%s' unknown, reporting as a crash",
                typestr);
    }
...
gboolean ubuntu_app_launch_observer_add_app_failed(
    UbuntuAppLaunchAppFailedObserver observer, gpointer user_data) {
  GDBusConnection *conn = g_bus_get_sync(G_BUS_TYPE_SESSION, NULL, NULL);
  ...
  observert->sighandle = g_dbus_connection_signal_subscribe(
      ...
      "/",                             /* path */
      NULL,                            /* arg0 */
      G_DBUS_SIGNAL_FLAGS_NONE, failed_signal_cb, observert,
      NULL); /* user data destroy */
...
static void paused_signal_cb(GDBusConnection *conn, const gchar *sender,
                             const gchar *object, const gchar *interface,
                             const gchar *signal, GVariant *params,
                             gpointer user_data) {
  ...
  if (observer->func != NULL) {
    ...
    while (g_variant_iter_loop(&thispid, "t", &pid)) {
      GPid gpid = (GPid)
          pid; /* Should be a no-op for most architectures, but just in case */
      g_array_append_val(pidarray, gpid);
    }

    observer->func(g_variant_get_string(appid, NULL), (GPid *)pidarray->data,
                   observer->user_data);
...
static gboolean paused_resumed_generic(
    UbuntuAppLaunchAppPausedResumedObserver observer, gpointer user_data,
    GList **queue, const gchar *signal_name, const gchar *lttng_signal) {
  GDBusConnection *conn = g_bus_get_sync(G_BUS_TYPE_SESSION, NULL, NULL);
  ...
  *queue = g_list_prepend(*queue, observert);
  ...
  observert->sighandle = g_dbus_connection_signal_subscribe(
      ...
      "/",                             /* path */
      NULL,                            /* arg0 */
      G_DBUS_SIGNAL_FLAGS_NONE, paused_signal_cb, observert,
      NULL); /* user data destroy */

  return TRUE;
}

gboolean ubuntu_app_launch_observer_add_app_paused(
    UbuntuAppLaunchAppPausedResumedObserver observer, gpointer user_data) {
  return paused_resumed_generic(observer, user_data, &paused_array,
                                "ApplicationPaused", "paused");
}

gboolean ubuntu_app_launch_observer_add_app_resumed(
    UbuntuAppLaunchAppPausedResumedObserver observer, gpointer user_data) {
  return paused_resumed_generic(observer, user_data, &resumed_array,
                                "ApplicationResumed", "resumed");
}

static gboolean delete_app_generic(UbuntuAppLaunchAppObserver observer,
                                   gpointer user_data, GList **list) {
...
gboolean ubuntu_app_launch_observer_delete_app_started(
    UbuntuAppLaunchAppObserver observer, gpointer user_data) {
  return delete_app_generic(observer, user_data, &started_array);
}

gboolean ubuntu_app_launch_observer_delete_app_stop(
    UbuntuAppLaunchAppObserver observer, gpointer user_data) {
  return delete_app_generic(observer, user_data, &stop_array);
}

gboolean ubuntu_app_launch_observer_delete_app_resume(
    UbuntuAppLaunchAppObserver observer, gpointer user_data) {
  return delete_app_generic(observer, user_data, &resume_array);
}

gboolean ubuntu_app_launch_observer_delete_app_focus(
    UbuntuAppLaunchAppObserver observer, gpointer user_data) {
  return delete_app_generic(observer, user_data, &focus_array);
}

gboolean ubuntu_app_launch_observer_delete_app_starting(
    UbuntuAppLaunchAppObserver observer, gpointer user_data) {
  return delete_app_generic(observer, user_data, &starting_array);
}

gboolean ubuntu_app_launch_observer_delete_app_failed(
    UbuntuAppLaunchAppFailedObserver observer, gpointer user_data) {
...
static gboolean paused_resumed_delete(
    UbuntuAppLaunchAppPausedResumedObserver observer, gpointer user_data,
    GList **list) {
...
gboolean ubuntu_app_launch_observer_delete_app_paused(
    UbuntuAppLaunchAppPausedResumedObserver observer, gpointer user_data) {
  return paused_resumed_delete(observer, user_data, &paused_array);
}

gboolean ubuntu_app_launch_observer_delete_app_resumed(
    UbuntuAppLaunchAppPausedResumedObserver observer, gpointer user_data) {
  return paused_resumed_delete(observer, user_data, &resumed_array);
}

typedef void (*per_instance_func_t)(GDBusConnection *con, GVariant *prop_dict,
                                    gpointer user_data);

static void foreach_job_instance(GDBusConnection *con, const gchar *jobname,
                                 per_instance_func_t func, gpointer user_data) {
  const gchar *job_path = get_jobpath(con, jobname);
  if (job_path == NULL) return;

  GError *error = NULL;
  GVariant *instance_tuple = g_dbus_connection_call_sync(
      con, DBUS_SERVICE_UPSTART, job_path, DBUS_INTERFACE_UPSTART_JOB,
      "GetAllInstances", NULL, G_VARIANT_TYPE("(ao)"), G_DBUS_CALL_FLAGS_NONE,
      -1,   /* timeout: default */
      NULL, /* cancelable */
      &error);

  if (error != NULL) {
    g_warning("Unable to get instances of job '%s': %s", jobname,
              error->message);
    g_error_free(error);
    return;
  }
  ...
  while (g_variant_iter_loop(&instance_iter, "&o", &instance_path)) {
    GVariant *props_tuple = g_dbus_connection_call_sync(
        con, DBUS_SERVICE_UPSTART, instance_path,
    ...
    if (error != NULL) {
      g_warning("Unable to name of instance '%s': %s", instance_path,
                error->message);
      g_error_free(error);
      error = NULL;
      continue;
...
static void apps_for_job_instance(GDBusConnection *con, GVariant *props_dict,
                                  gpointer user_data) {
  GVariant *namev =
      g_variant_lookup_value(props_dict, "name", G_VARIANT_TYPE_STRING);
  if (namev == NULL) {
    return;
  }
  ...
  if (data->truncate_legacy &&
      g_strcmp0(data->jobname, "application-legacy") == 0) {
...
static void apps_for_job(GDBusConnection *con, const gchar *jobname,
                         GArray *apps, gboolean truncate_legacy) {
  apps_for_job_t data = {
      .jobname = jobname, .apps = apps, .truncate_legacy = truncate_legacy};
...
static void pid_for_job_instance(GDBusConnection *con, GVariant *props_dict,
                                 gpointer user_data) {
  GVariant *namev =
      g_variant_lookup_value(props_dict, "name", G_VARIANT_TYPE_STRING);
  if (namev == NULL) {
  ...
  if (g_strcmp0(instance_name, data->appid) == 0) {
    GVariant *processv = g_variant_lookup_value(props_dict, "processes",
                                                G_VARIANT_TYPE("a(si)"));
...
static GPid pid_for_job(GDBusConnection *con, const gchar *jobname,
                        const gchar *appid) {
  pid_for_job_t data = {.jobname = jobname, .appid = appid, .pid = 0};
...
GList *ubuntu_app_launch_get_pids(const gchar *appid) {
  ...
  for (i = 0; i < apps->len; i++) {
    ...
    if (g_str_has_prefix(array_id, appiddash)) {
      GList *morepids =
          pids_from_cgroup(cgmanager, "application-legacy", array_id);
...
gboolean ubuntu_app_launch_app_id_parse(const gchar *appid, gchar **package,
                                        gchar **application, gchar **version) {
  g_return_val_if_fail(appid != NULL, FALSE);
...
gchar *ubuntu_app_launch_triplet_to_app_id(const gchar *pkg, const gchar *app,
                                           const gchar *ver) {
  g_return_val_if_fail(pkg != NULL, NULL);

  /* Check if is a libertine container */
  gchar *libertinepath = g_build_filename(g_get_user_cache_dir(),
                                          "libertine-container", pkg, NULL);
...
static void start_helper_callback(GObject *obj, GAsyncResult *res,
                                  gpointer user_data) {
  GError *error = NULL;
  GVariant *result = NULL;
...
static gboolean start_helper_core(const gchar *type, const gchar *appid,
                                  const gchar *const *uris,
                                  const gchar *instance,
                                  const gchar *mirsocketpath) {
  GDBusConnection *con = g_bus_get_sync(G_BUS_TYPE_SESSION, NULL, NULL);
  g_return_val_if_fail(con != NULL, FALSE);
  ...
  g_variant_builder_add_value(
      &builder,
      g_variant_new_take_string(g_strdup_printf("HELPER_TYPE=%s", type)));

  if (uris != NULL) {
    gchar *urisjoin = app_uris_string(uris);
    g_variant_builder_add_value(
        &builder,
        g_variant_new_take_string(g_strdup_printf("APP_URIS=%s", urisjoin)));
    g_free(urisjoin);
  }
  ...
  if (instance != NULL) {
    g_variant_builder_add_value(
        &builder,
        g_variant_new_take_string(g_strdup_printf("INSTANCE_ID=%s", instance)));
  }

  if (mirsocketpath != NULL) {
    g_variant_builder_add_value(
        &builder, g_variant_new_take_string(g_strdup_printf(
                      "UBUNTU_APP_LAUNCH_DEMANGLE_PATH=%s", mirsocketpath)));
    g_variant_builder_add_value(
        &builder, g_variant_new_take_string(
                      g_strdup_printf("UBUNTU_APP_LAUNCH_DEMANGLE_NAME=%s",
                                      g_dbus_connection_get_unique_name(con))));
  }
...
gboolean ubuntu_app_launch_start_helper(const gchar *type, const gchar *appid,
                                        const gchar *const *uris) {
...
static void get_mir_session_fd_helper(MirPromptSession *session, size_t count,
                                      int const *fdin, void *user_data) {
  if (count != 1) {
...
static int get_mir_session_fd(MirPromptSession *session) {
  int retfd = 0;
  MirWaitHandle *wait = mir_prompt_session_new_fds_for_prompt_providers(
      session, 1, get_mir_session_fd_helper, &retfd);
...
static gint remove_socket_path_find(gconstpointer a, gconstpointer b) {
...
static gboolean remove_socket_path(const gchar *path) {
  GList *thisproxy =
      g_list_find_custom(open_proxies, path, remove_socket_path_find);
  if (thisproxy == NULL) return FALSE;
  ...
  if (mirfd != 0) {
    ...
    const gchar *props[3] = {"UbuntuAppLaunchProxyDbusPath", NULL, NULL};
    props[1] = path;
    report_recoverable_problem("ubuntu-app-launch-mir-fd-proxy", 0, TRUE,
                               props);
  }
...
static gboolean proxy_timeout(gpointer user_data) {
...
static void proxy_cleanup_list(void) {
  while (open_proxies) {
...
static gboolean proxy_mir_socket(GObject *obj,
  ...
  if (error == NULL) {
    g_dbus_method_invocation_return_value_with_unix_fd_list(invocation, tuple,
                                                            list);
  } else {
...
static gchar *build_proxy_socket_path(const gchar *appid, int mirfd) {
  ...
  g_signal_connect(G_OBJECT(skel), "handle-get-mir-socket",
                   G_CALLBACK(proxy_mir_socket), GINT_TO_POINTER(mirfd));
  ...
  while (socket_name == NULL) {
    gchar *tryname = g_strdup_printf("/com/canonical/UbuntuAppLaunch/%s/%X",
                                     encoded_appid, g_random_int());
    g_dbus_interface_skeleton_export(G_DBUS_INTERFACE_SKELETON(skel), session,
                                     tryname, &error);
    ...
    if (error == NULL) {
      ...
      bool exitnow = (error->domain != G_DBUS_ERROR ||
                      error->code != G_DBUS_ERROR_OBJECT_PATH_IN_USE);
  ...
  g_object_set_qdata_full(G_OBJECT(skel), proxy_path_quark(),
                          g_strdup(socket_name), g_free);
  g_object_set_qdata(G_OBJECT(skel), mir_fd_quark(), GINT_TO_POINTER(mirfd));
  open_proxies = g_list_prepend(open_proxies, skel);

  g_timeout_add_seconds_full(G_PRIORITY_DEFAULT, 2, proxy_timeout,
                             g_strdup(socket_name), g_free);
...
static void stop_helper_callback(GObject *obj, GAsyncResult *res,
                                 gpointer user_data) {
  GError *error = NULL;
  GVariant *result = NULL;
...
static gboolean stop_helper_core(const gchar *type, const gchar *appid,
  ...
  /* Call the job start function */
  g_dbus_connection_call(con, DBUS_SERVICE_UPSTART, jobpath,
                         DBUS_INTERFACE_UPSTART_JOB, "Stop",
                         g_variant_builder_end(&builder), NULL,
                         G_DBUS_CALL_FLAGS_NONE, -1, NULL, /* cancelable */
                         stop_helper_callback, NULL);
...
static void list_helpers_helper(GDBusConnection *con, GVariant *props_dict,
                                gpointer user_data) {
...
gchar **ubuntu_app_launch_list_helpers(const gchar *type) {
  ...
  foreach_job_instance(con, "untrusted-helper", list_helpers_helper,
                       &helpers_helper_data);
...
static void list_helper_instances(GDBusConnection *con, GVariant *props_dict,
                                  gpointer user_data) {
...
gchar **ubuntu_app_launch_list_helper_instances(const gchar *type,
  ...
  foreach_job_instance(con, "untrusted-helper", list_helper_instances,
                       &helper_instances_data);
...
static void helper_observer_cb(GDBusConnection *conn, const gchar *sender,
                               const gchar *object, const gchar *interface,
                               const gchar *signal, GVariant *params,
                               gpointer user_data) {
...
/* Creates the observer structure and registers for the signal with
   GDBus so that we can get a callback */
static gboolean add_helper_generic(UbuntuAppLaunchHelperObserver observer,
                                   const gchar *helper_type, gpointer user_data,
                                   const gchar *signal, GList **list) {
  GDBusConnection *conn = gdbus_upstart_ref();
  ...
  *list = g_list_prepend(*list, observert);
  ...
  observert->sighandle = g_dbus_connection_signal_subscribe(
      ...
      DBUS_PATH_UPSTART,      /* path */
      signal,                 /* arg0 */
      G_DBUS_SIGNAL_FLAGS_NONE, helper_observer_cb, observert,
      NULL); /* user data destroy */

  return TRUE;
}

static gboolean delete_helper_generic(UbuntuAppLaunchHelperObserver observer,
  ...
  for (look = *list; look != NULL; look = g_list_next(look)) {
    ...
    if (observert->func == observer && observert->user_data == user_data &&
        g_str_has_prefix(observert->type, type)) {
      break;
    }
  }
...
gboolean ubuntu_app_launch_observer_add_helper_started(
    UbuntuAppLaunchHelperObserver observer, const gchar *helper_type,
    gpointer user_data) {
  ...
  return add_helper_generic(observer, helper_type, user_data, "started",
                            &helper_started_obs);
}

gboolean ubuntu_app_launch_observer_add_helper_stop(
    UbuntuAppLaunchHelperObserver observer, const gchar *helper_type,
    gpointer user_data) {
  g_return_val_if_fail(observer != NULL, FALSE);
  g_return_val_if_fail(helper_type != NULL, FALSE);
  ...
  return add_helper_generic(observer, helper_type, user_data, "stopped",
                            &helper_stopped_obs);
}

gboolean ubuntu_app_launch_observer_delete_helper_started(
    UbuntuAppLaunchHelperObserver observer, const gchar *helper_type,
    gpointer user_data) {
  g_return_val_if_fail(observer != NULL, FALSE);
  g_return_val_if_fail(helper_type != NULL, FALSE);
  ...
  return delete_helper_generic(observer, helper_type, user_data,
                               &helper_started_obs);
}

gboolean ubuntu_app_launch_observer_delete_helper_stop(
    UbuntuAppLaunchHelperObserver observer, const gchar *helper_type,
    gpointer user_data) {
  g_return_val_if_fail(observer != NULL, FALSE);
  g_return_val_if_fail(helper_type != NULL, FALSE);
...
static void set_var(GDBusConnection *bus, const gchar *job_name,
                    const gchar *instance_name, const gchar *envvar) {
  ...
  g_dbus_connection_call(bus, "com.ubuntu.Upstart", "/com/ubuntu/Upstart",
                         "com.ubuntu.Upstart0_6", "SetEnv",
                         g_variant_builder_end(&builder), NULL, /* reply */

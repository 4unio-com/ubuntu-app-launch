...
IconFinder::IconFinder(std::string basePath)
    : _searchPaths(getSearchPaths(basePath)), _basePath(basePath) {}
...
/** Check in a given path if there is an existing file in it that satifies our
 * name */
std::string IconFinder::findExistingIcon(const std::string& path,
  ...
  for (const auto& extension : ICON_TYPES) {
    auto fullpath =
        g_build_filename(path.c_str(), (iconName + extension).c_str(), nullptr);
    if (g_file_test(fullpath, G_FILE_TEST_EXISTS)) {
      iconPath = fullpath;
      g_free(fullpath);
      break;
...
std::list<IconFinder::ThemeSubdirectory> IconFinder::validDirectories(
    const std::string& themePath, gchar* directory, int size) {
  std::list<IconFinder::ThemeSubdirectory> dirs;
  auto globalHicolorTheme =
      g_build_filename(themePath.c_str(), directory, nullptr);
...
std::list<IconFinder::ThemeSubdirectory> IconFinder::addSubdirectoryByType(
    std::shared_ptr<GKeyFile> themefile, gchar* directory,
    const std::string& themePath) {
  GError* error = nullptr;
  auto gType =
      g_key_file_get_string(themefile.get(), directory, TYPE_PROPERTY, &error);
  if (error != nullptr) {
    g_error_free(error);
  ...
  if (type == FIXED_CONTEXT) {
    auto size = g_key_file_get_integer(themefile.get(), directory,
                                       SIZE_PROPERTY, &error);
    if (error != nullptr) {
      g_error_free(error);
    } else {
    ...
    if (error != nullptr) {
      ...
      auto threshold = g_key_file_get_integer(themefile.get(), directory,
                                              THRESHOLD_PROPERTY, &error);
      if (error != nullptr) {
...
std::list<IconFinder::ThemeSubdirectory> IconFinder::searchIconPaths(
    std::shared_ptr<GKeyFile> themefile, gchar** directories,
    const std::string& themePath) {
  std::list<ThemeSubdirectory> subdirs;
  for (auto i = 0; directories[i] != nullptr; ++i) {
    GError* error = nullptr;
    auto context = g_key_file_get_string(themefile.get(), directories[i],
                                         CONTEXT_PROPERTY, &error);
    if (error != nullptr) {
      g_error_free(error);
      continue;
    }
    if (g_strcmp0(context, APPLICATIONS_TYPE) == 0) {
      auto newDirs =
          addSubdirectoryByType(themefile, directories[i], themePath);
...
/** Try to get theme subdirectories using .theme file in the given theme path
    if it exists */
std::list<IconFinder::ThemeSubdirectory> IconFinder::themeFileSearchPaths(
    const std::string& themePath) {
  auto themeFilePath =
      g_build_filename(themePath.c_str(), THEME_INDEX_FILE, nullptr);
  GError* error = nullptr;
  auto themefile = std::shared_ptr<GKeyFile>(g_key_file_new(), g_key_file_free);
  g_key_file_load_from_file(themefile.get(), themeFilePath, G_KEY_FILE_NONE,
                            &error);
  if (error != nullptr) {
  ...
  auto directories = g_key_file_get_string_list(
      themefile.get(), ICON_THEME_KEY, DIRECTORIES_PROPERTY, nullptr, &error);
  if (error != nullptr) {
...
std::list<IconFinder::ThemeSubdirectory> IconFinder::themeDirSearchPaths(
  ...
  if (error != nullptr) {
    g_debug("Unable to open directory '%s' becuase: %s", themeDir.c_str(),
            error->message);
    g_error_free(error);
    return searchPaths;
  }
  ...
  while ((dirname = g_dir_read_name(gdir)) != nullptr) {
    auto fullPath =
        g_build_filename(themeDir.c_str(), dirname, "apps", nullptr);
    /* Directories only */
    if (g_file_test(fullPath, G_FILE_TEST_IS_DIR)) {
      ...
      /* We want it to match and have the same values for the first and second
       * size */
      if (std::regex_match(dirstr, match, iconSizeDirname)) {
        searchPaths.emplace_back(IconFinder::ThemeSubdirectory{
            fullPath, std::atoi(match[1].str().c_str())});
      }
    }
    g_free(fullPath);
  }
...
std::list<IconFinder::ThemeSubdirectory> IconFinder::iconsFromThemePath(
  ...
  if (g_file_test(themeDir, G_FILE_TEST_IS_DIR)) {
    ...
    if (themeDirs.size() > 0) {
      ...
      /* If we didn't get from a theme file, we need to manually scan the
       * directories */
...
/** Gets search paths based on common icon directories including themes and
 * pixmaps. */
std::list<IconFinder::ThemeSubdirectory> IconFinder::getSearchPaths(
    const std::string& basePath) {
  std::list<IconFinder::ThemeSubdirectory> iconPaths;

  /* Icons from the hicolor theme */
  auto hicolorDir =
      g_build_filename(basePath.c_str(), HICOLOR_THEME_DIR, nullptr);
  ...
  auto humanityDir =
      g_build_filename(basePath.c_str(), HUMANITY_THEME_DIR, nullptr);
  ...
  iconPaths.sort(
      [](const ThemeSubdirectory& lhs, const ThemeSubdirectory& rhs) {

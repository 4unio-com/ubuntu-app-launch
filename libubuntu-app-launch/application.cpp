...
std::shared_ptr<Application> Application::create(
    const AppID& appid, const std::shared_ptr<Registry>& registry) {
  if (appid.empty()) {
  ...
  if (app_info_click(sappid.c_str(), NULL, NULL)) {
    ...
    return std::make_shared<app_impls::Libertine>(appid.package, appid.appname,
                                                  registry);
...
AppID AppID::parse(const std::string& sappid) {
  ...
  if (std::regex_match(sappid, match, full_appid_regex)) {
    ...
    return {AppID::Package::from_raw({}), AppID::AppName::from_raw({}),
...
AppID AppID::find(const std::string& sappid) {
  ...
  if (std::regex_match(sappid, match, full_appid_regex)) {
    ...
    return {AppID::Package::from_raw({}), AppID::AppName::from_raw(sappid),
            AppID::Version::from_raw({})};
  } else {
    return {AppID::Package::from_raw({}), AppID::AppName::from_raw({}),
            AppID::Version::from_raw({})};
...
bool operator==(const AppID& a, const AppID& b) {
  return a.package.value() == b.package.value() &&
         a.appname.value() == b.appname.value() &&
...
bool AppID::empty() const {
  return package.value().empty() && appname.value().empty() &&
         version.value().empty();
}
...
AppID AppID::discover(const std::string& package, const std::string& appname,
                      const std::string& version) {
  auto cappid = ubuntu_app_launch_triplet_to_app_id(
      package.c_str(), appname.c_str(), version.c_str());
...
AppID AppID::discover(const std::string& package,
                      ApplicationWildcard appwildcard,
                      VersionWildcard versionwildcard) {
  return discover(package, app_wildcard(appwildcard),
                  ver_wildcard(versionwildcard));
}
...
AppID AppID::discover(const std::string& package, const std::string& appname,
  ...
  if (appid.empty()) {
    ...
    if (app_info_libertine((package + "_" + appname + "_0.0").c_str(), nullptr,
                           nullptr)) {
      appid = AppID(Package::from_raw(package), AppName::from_raw(appname),
                    Version::from_raw("0.0"));
    }
  }

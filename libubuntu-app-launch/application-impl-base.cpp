...
Base::Base(const std::shared_ptr<Registry>& registry) : _registry(registry) {}
...
class BaseInstance : public Application::Instance {
 public:
  ...
  /* Manage lifecycle */
  void pause() override { ubuntu_app_launch_pause_application(_appId.c_str()); }
  void resume() override {
    ubuntu_app_launch_resume_application(_appId.c_str());
  }
  void stop() override { ubuntu_app_launch_stop_application(_appId.c_str()); }

 private:
  std::string _appId;
};

BaseInstance::BaseInstance(const std::string& appId) : _appId(appId) {}
...
std::shared_ptr<Application::Instance> Base::launch(
...
std::shared_ptr<Application::Instance> Base::launchTest(
    const std::vector<Application::URL>& urls) {
  std::string appIdStr = appId();
  std::shared_ptr<gchar*> urlstrv;

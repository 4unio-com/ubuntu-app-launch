...
static GSource* thread_default_timeout(guint interval, GSourceFunc func, gpointer data)
{
...
static void unity_resume_cb(GDBusConnection* connection,
                            const gchar* sender,
                            const gchar* path,
...
static void app_id_to_dbus_path(second_exec_t* data)
{
    ...
    {
        if ((data->appid[i] >= 'a' && data->appid[i] <= 'z') || (data->appid[i] >= 'A' && data->appid[i] <= 'Z') ||
...
static void send_open_cb(GObject* object, GAsyncResult* res, gpointer user_data)
{
    GError* error = NULL;
...
static void contact_app(GDBusConnection* bus, const gchar* dbus_name, second_exec_t* data)
{
    ...
    g_dbus_connection_call(bus, dbus_name, data->dbus_path, "org.freedesktop.Application", "Open", data->app_data, NULL,
                           G_DBUS_CALL_FLAGS_NONE, -1, NULL, send_open_cb, data);
...
static void find_appid_pid(GDBusConnection* session, second_exec_t* data)
{
    GError* error = NULL;
    ...
    GVariant* listnames =
        g_dbus_connection_call_sync(session, "org.freedesktop.DBus", "/", "org.freedesktop.DBus", "ListNames", NULL,
    ...
    while (g_variant_iter_loop(&iter, "s", &name))
    {
        ...
        /* Get the PIDs */
        g_dbus_connection_call(session, "org.freedesktop.DBus", "/", "org.freedesktop.DBus",
                               "GetConnectionUnixProcessID", g_variant_new("(s)", name), G_VARIANT_TYPE("(u)"),
                               G_DBUS_CALL_FLAGS_NONE, -1, NULL, get_pid_cb, pid_data);
...
gboolean second_exec(GDBusConnection* session,
                     GCancellable* cancel,
                     GPid pid,
                     const gchar* app_id,
                     ...
                     gchar** appuris)
{
    ...
    data->signal = g_dbus_connection_signal_subscribe(session, NULL,                   /* sender */
...
static void second_exec_complete(second_exec_t* data)
{
    ...
    g_dbus_connection_emit_signal(data->bus, NULL,                 /* destination */
                                  ...
                                  g_variant_new("(ss)", data->appid, data->instanceid), &error);

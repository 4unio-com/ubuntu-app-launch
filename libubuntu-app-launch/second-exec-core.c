...
static gboolean timer_cb(gpointer user_data) {
  ual_tracepoint(second_exec_resume_timeout,
                 ((second_exec_t *)user_data)->appid);
...
static void connection_count_dec(second_exec_t *data) {
...
static void unity_resume_cb(GDBusConnection *connection, const gchar *sender,
...
static void parse_uris(second_exec_t *data) {
  ...
  if (uri_split == NULL || uri_split[0] == NULL || error != NULL) {
    if (error != NULL) {
      g_warning("Unable to parse URLs '%s': %s", data->input_uris,
                error->message);
      g_error_free(error);
    }
    ...
    for (i = 0; uri_split[i] != NULL; i++) {
      g_variant_builder_add_value(&builder,
                                  g_variant_new_take_string(uri_split[i]));
    }
    g_free(uri_split);
...
*/
static void app_id_to_dbus_path(second_exec_t *data) {
  if (data->dbus_path != NULL) {
    return;
  }

  GString *str = g_string_sized_new(
      strlen(data->appid) + 2); /* base case, we just need a / and a null */
...
static void send_open_cb(GObject *object, GAsyncResult *res,
                         gpointer user_data) {
  GError *error = NULL;

  ual_tracepoint(second_exec_app_contacted,
                 ((second_exec_t *)user_data)->appid);
...
static void contact_app(GDBusConnection *bus, const gchar *dbus_name,
                        second_exec_t *data) {
  ...
  g_dbus_connection_call(bus, dbus_name, data->dbus_path,
                         "org.freedesktop.Application", "Open", data->app_data,
                         NULL, G_DBUS_CALL_FLAGS_NONE, -1, NULL, send_open_cb,
                         data);
...
static void get_pid_cb(GObject *object, GAsyncResult *res, gpointer user_data) {
  ...
  if (error != NULL) {
    g_warning("Unable to query PID for dbus name '%s': %s", data->name,
              error->message);
    g_error_free(error);
...
void find_appid_pid(GDBusConnection *session, second_exec_t *data) {
  GError *error = NULL;
  ...
  GVariant *listnames = g_dbus_connection_call_sync(
      session, "org.freedesktop.DBus", "/", "org.freedesktop.DBus", "ListNames",
      NULL, G_VARIANT_TYPE("(as)"), G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);
  ...
  /* Next figure out what we're looking for (and if there is something to look
   * for) */
  ...
  while (g_variant_iter_loop(&iter, "s", &name)) {
    ...
    /* Get the PIDs */
    g_dbus_connection_call(session, "org.freedesktop.DBus", "/",
                           "org.freedesktop.DBus", "GetConnectionUnixProcessID",
                           g_variant_new("(s)", name), G_VARIANT_TYPE("(u)"),
                           G_DBUS_CALL_FLAGS_NONE, -1, NULL, get_pid_cb,
                           pid_data);
...
gboolean second_exec(const gchar *app_id, const gchar *appuris) {
  ...
  data->signal = g_dbus_connection_signal_subscribe(
      session, NULL,                   /* sender */
      "com.canonical.UbuntuAppLaunch", /* interface */
      "UnityResumeResponse",           /* signal */
      "/",                             /* path */
      app_id,                          /* arg0 */
      G_DBUS_SIGNAL_FLAGS_NONE, unity_resume_cb, data,
...
static void second_exec_complete(second_exec_t *data) {
  ...
  g_dbus_connection_emit_signal(data->bus, NULL, /* destination */
                                "/",             /* path */
                                "com.canonical.UbuntuAppLaunch", /* interface */
                                "UnityFocusRequest",             /* signal */
                                g_variant_new("(s)", data->appid), &error);
  ...
  if (data->app_data != NULL) g_variant_unref(data->app_data);
  g_free(data->appid);

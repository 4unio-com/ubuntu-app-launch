...
void report_error_on_caller(const gchar* app_id) {
  g_warning("Unable to find keyfile for application '%s'", app_id);

  const gchar* props[3] = {"AppId", NULL, NULL};
  props[1] = app_id;
  ...
  if (!debugtool) {
    report_recoverable_problem("ubuntu-app-launch-invalid-appid", pid, TRUE,
                               props);
  } else {
...
/* Get the keyfile object for a libertine container based application. Look into
   the container's filesystem on disk and find it in /usr/share/applications in
   there.
   Those are currently the only apps that we look at today. We're not ensuring
   anything
   about the file other than it has basic sanity. */
GKeyFile* keyfile_for_libertine(const gchar* appid, gchar** outcontainer) {
  ...
  gboolean loaded =
      g_key_file_load_from_file(keyfile, desktopfull, G_KEY_FILE_NONE, NULL);
...
gboolean desktop_task_setup(GDBusConnection* bus, const gchar* app_id,
                            EnvHandle* handle, gboolean is_libertine) {
  if (app_id == NULL) {
  ...
  gchar* apparmor = g_key_file_get_string(keyfile, "Desktop Entry",
                                          "X-Ubuntu-AppArmor-Profile", NULL);
  if (apparmor != NULL) {
  ...
  if (g_key_file_has_key(keyfile, "Desktop Entry", "X-Ubuntu-XMir-Enable",
                         NULL)) {
    if (g_key_file_get_boolean(keyfile, "Desktop Entry", "X-Ubuntu-XMir-Enable",
                               NULL)) {
  ...
  if (is_libertine) {
    ...
    gchar* libexec = g_strdup_printf("%s \"%s\" %s", libertine_launch,
                                     libertinecontainer, execline);
    g_free(execline);
    execline = libexec;
  }

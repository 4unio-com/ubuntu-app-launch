...
class Application {
 public:
  ...
  /** Function to create an Application object. It determines the type of
     application
   ...
   public:
    ...
    struct Orientations {
      ...
      bool operator==(const Orientations& b) const {
        return portrait == b.portrait && landscape == b.landscape &&
               invertedPortrait == b.invertedPortrait &&
  ...
  */
  virtual std::shared_ptr<Instance> launch(
      const std::vector<URL>& urls = {}) = 0;
  /** Start an application with text flags, optionally with URLs to
      pass to it.

      \param urls A list of URLs to pass to the application command line
  */
  virtual std::shared_ptr<Instance> launchTest(
      const std::vector<URL>& urls = {}) = 0;
};

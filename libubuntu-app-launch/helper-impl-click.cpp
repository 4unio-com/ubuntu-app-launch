...
class ClickInstance : public Helper::Instance {
 public: /* all one file, no one to hide from */
  ...
  ClickInstance(const AppID& appid, const Helper::Type& type,
  ...
  void stop() override {
    _registry->impl->thread.executeOnThread<bool>([this]() {
      return ubuntu_app_launch_stop_multiple_helper(
                 _type.value().c_str(), ((std::string)_appid).c_str(),
...
std::vector<std::shared_ptr<Click::Instance>> Click::instances() {
  return _registry->impl->thread
      .executeOnThread<std::vector<std::shared_ptr<Click::Instance>>>(
          [this]() -> std::vector<std::shared_ptr<Click::Instance>> {
            std::vector<std::shared_ptr<Click::Instance>> vect;
            auto instances = ubuntu_app_launch_list_helper_instances(
                _type.value().c_str(), ((std::string)_appid).c_str());
            for (int i = 0; instances[i] != nullptr; i++) {
              auto inst = std::make_shared<ClickInstance>(
                  _appid, _type, instances[i], _registry);
              vect.push_back(inst);
            }
...
std::shared_ptr<gchar*> urlsToStrv(std::vector<Helper::URL> urls) {
  ...
  return std::shared_ptr<gchar*>((gchar**)g_array_free(array, FALSE),
                                 g_strfreev);
}
...
std::shared_ptr<Click::Instance> Click::launch(std::vector<Helper::URL> urls) {
  ...
  return _registry->impl->thread
      .executeOnThread<std::shared_ptr<Click::Instance>>([this, urlstrv]() {
        auto instanceid = ubuntu_app_launch_start_multiple_helper(
            _type.value().c_str(), ((std::string)_appid).c_str(),
            urlstrv.get());
...
std::shared_ptr<Click::Instance> Click::launch(MirPromptSession* session,
                                               std::vector<Helper::URL> urls) {
  auto urlstrv = urlsToStrv(urls);

  return _registry->impl->thread
      .executeOnThread<std::shared_ptr<Click::Instance>>(
          [this, session, urlstrv]() {
            auto instanceid = ubuntu_app_launch_start_session_helper(
                _type.value().c_str(), session, ((std::string)_appid).c_str(),
                urlstrv.get());
...
std::list<std::shared_ptr<Helper>> Click::running(
    Helper::Type type, std::shared_ptr<Registry> registry) {
  return registry->impl->thread
      .executeOnThread<std::list<std::shared_ptr<Helper>>>([type, registry]() {
        ...
        for (int i = 0; appidv[i] != nullptr; i++) {
          auto helper =
              std::make_shared<Click>(type, AppID::parse(appidv[i]), registry);

...
static const gchar* get_jobpath(GDBusConnection* con, const gchar* jobname)
{
    ...
    GVariant* job_path_variant = g_dbus_connection_call_sync(
        con, DBUS_SERVICE_UPSTART, DBUS_PATH_UPSTART, DBUS_INTERFACE_UPSTART, "GetJobByName",
        g_variant_new("(s)", jobname), G_VARIANT_TYPE("(o)"), G_DBUS_CALL_FLAGS_NONE, -1, /* timeout: default */
        NULL,                                                                             /* cancellable */
        &error);
...
static void executeOnContext(const std::shared_ptr<GMainContext>& context, std::function<void()> work)
{
    ...
    auto source =
        std::shared_ptr<GSource>(g_idle_source_new(), [](GSource* src) { g_clear_pointer(&src, g_source_unref); });
...
template <core::Signal<
    const std::shared_ptr<ubuntu::app_launch::Application>&,
    const std::shared_ptr<ubuntu::app_launch::Application::
                              Instance>&>& (*getSignal)(const std::shared_ptr<ubuntu::app_launch::Registry>&)>
static gboolean observer_add(
    UbuntuAppLaunchAppObserver observer,
    gpointer user_data,
    std::map<std::pair<UbuntuAppLaunchAppObserver, gpointer>, core::ScopedConnection>& observers)
{
    auto context = std::shared_ptr<GMainContext>(g_main_context_ref_thread_default(), [](GMainContext* context) {
        g_clear_pointer(&context, g_main_context_unref);
    });

    observers.emplace(std::make_pair(
        std::make_pair(observer, user_data),
        core::ScopedConnection(getSignal(ubuntu::app_launch::Registry::getDefault())
                                   .connect([context, observer, user_data](
...
static gboolean observer_delete(observertype observer,
                                gpointer user_data,
                                std::map<std::pair<observertype, gpointer>, core::ScopedConnection>& observers)
{
...
private:
    ...
    struct ObserverData
    {
        ...
        ObserverData(UbuntuAppLaunchAppObserver obs, gpointer ud)
            ...
            , user_data(ud)
        {
            context = std::shared_ptr<GMainContext>(g_main_context_ref_thread_default(), [](GMainContext* context) {
                g_clear_pointer(&context, g_main_context_unref);
            });
        }
    };
                            ...
                            std::list<ObserverData>& list)
    {
        ...
        {
            executeOnContext(data.context, [data, sappid]() { data.observer(sappid.c_str(), data.user_data); });
        }
...
static std::shared_ptr<CManager> ensure_cmanager()
{
...
gboolean ubuntu_app_launch_observer_add_app_failed(UbuntuAppLaunchAppFailedObserver observer, gpointer user_data)
{
    ...
    appFailedObservers.emplace(std::make_pair(
        std::make_pair(observer, user_data),
        core::ScopedConnection(ubuntu::app_launch::Registry::appFailed().connect(
            [context, observer, user_data](std::shared_ptr<ubuntu::app_launch::Application> app,
                                           std::shared_ptr<ubuntu::app_launch::Application::Instance> instance,
...
template <core::Signal<const std::shared_ptr<ubuntu::app_launch::Application>&,
                       const std::shared_ptr<ubuntu::app_launch::Application::Instance>&,
                       const std::vector<pid_t>&>& (*getSignal)(const std::shared_ptr<ubuntu::app_launch::Registry>&)>
static gboolean observer_add_pause(
    UbuntuAppLaunchAppPausedResumedObserver observer,
    gpointer user_data,
    std::map<std::pair<UbuntuAppLaunchAppPausedResumedObserver, gpointer>, core::ScopedConnection>& observers)
{
    auto context = std::shared_ptr<GMainContext>(g_main_context_ref_thread_default(), [](GMainContext* context) {
        g_clear_pointer(&context, g_main_context_unref);
    });
...
static std::map<std::pair<UbuntuAppLaunchAppPausedResumedObserver, gpointer>, core::ScopedConnection>
    appPausedObservers;

gboolean ubuntu_app_launch_observer_add_app_paused(UbuntuAppLaunchAppPausedResumedObserver observer, gpointer user_data)
{
...
static void foreach_job_instance(GDBusConnection* con,
                                 const gchar* jobname,
                                 per_instance_func_t func,
                                 gpointer user_data)
{
    ...
    GVariant* instance_tuple =
        g_dbus_connection_call_sync(con, DBUS_SERVICE_UPSTART, job_path, DBUS_INTERFACE_UPSTART_JOB, "GetAllInstances",
                                    NULL, G_VARIANT_TYPE("(ao)"), G_DBUS_CALL_FLAGS_NONE, -1, /* timeout: default */
                                    NULL,                                                     /* cancellable */
                                    &error);
    ...
    while (g_variant_iter_loop(&instance_iter, "&o", &instance_path))
    {
        GVariant* props_tuple =
            g_dbus_connection_call_sync(con, DBUS_SERVICE_UPSTART, instance_path, "org.freedesktop.DBus.Properties",
                                        "GetAll", g_variant_new("(s)", DBUS_INTERFACE_UPSTART_INSTANCE),
                                        G_VARIANT_TYPE("(a{sv})"), G_DBUS_CALL_FLAGS_NONE, -1, /* timeout: default */
                                        NULL,                                                  /* cancellable */
                                        &error);
...
gchar* ubuntu_app_launch_triplet_to_app_id(const gchar* pkg, const gchar* app, const gchar* ver)
{
...
static void start_helper_callback(GObject* obj, GAsyncResult* res, gpointer user_data)
{
    GError* error = NULL;
...
static gboolean start_helper_core(
    const gchar* type, const gchar* appid, const gchar* const* uris, const gchar* instance, const gchar* mirsocketpath)
{
    ...
    if (mirsocketpath != NULL)
    {
        g_variant_builder_add_value(
            &builder, g_variant_new_take_string(g_strdup_printf("UBUNTU_APP_LAUNCH_DEMANGLE_PATH=%s", mirsocketpath)));
        g_variant_builder_add_value(&builder,
                                    g_variant_new_take_string(g_strdup_printf("UBUNTU_APP_LAUNCH_DEMANGLE_NAME=%s",
    ...
    g_dbus_connection_call(con, DBUS_SERVICE_UPSTART, jobpath, DBUS_INTERFACE_UPSTART_JOB, "Start",
                           g_variant_builder_end(&builder), NULL, G_DBUS_CALL_FLAGS_NONE, -1, NULL, /* cancellable */
...
static void get_mir_session_fd_helper(MirPromptSession* session, size_t count, int const* fdin, void* user_data)
{
...
static int get_mir_session_fd(MirPromptSession* session)
{
    ...
    mir_prompt_session_new_fds_for_prompt_providers(session, 1, get_mir_session_fd_helper, &promise);
...
static gboolean proxy_mir_socket(GObject* obj, GDBusMethodInvocation* invocation, gpointer user_data)
{
...
static gchar* build_proxy_socket_path(const gchar* appid, int mirfd)
{
    ...
    while (socket_name == NULL)
    {
        ...
        g_dbus_interface_skeleton_export(G_DBUS_INTERFACE_SKELETON(skel), session, tryname, &error);
    ...
    g_timeout_add_seconds_full(G_PRIORITY_DEFAULT, 2, proxy_timeout, g_strdup(socket_name), g_free);
...
static void stop_helper_callback(GObject* obj, GAsyncResult* res, gpointer user_data)
{
    GError* error = NULL;
    GVariant* result = NULL;
...
static gboolean stop_helper_core(const gchar* type, const gchar* appid, const gchar* instanceid)
{
    GDBusConnection* con = g_bus_get_sync(G_BUS_TYPE_SESSION, NULL, NULL);
    g_return_val_if_fail(con != NULL, FALSE);
    ...
    /* Call the job start function */
    g_dbus_connection_call(con, DBUS_SERVICE_UPSTART, jobpath, DBUS_INTERFACE_UPSTART_JOB, "Stop",
                           g_variant_builder_end(&builder), NULL, G_DBUS_CALL_FLAGS_NONE, -1, NULL, /* cancellable */
...
static void list_helpers_helper(GDBusConnection* con, GVariant* props_dict, gpointer user_data)
{
...
gchar** ubuntu_app_launch_list_helpers(const gchar* type)
{
    ...
    helpers_helper_t helpers_helper_data = {g_strdup_printf("%s:", type), strlen(type) + 1, /* 1 for the colon */
...
static void list_helper_instances(GDBusConnection* con, GVariant* props_dict, gpointer user_data)
{
    ...
    if (g_str_has_prefix(name, data->type_prefix) && (suffix_loc = g_strrstr(name, data->appid_suffix)) != NULL)
    {
        /* Skip the type name */
        name += data->type_len;
...
gchar** ubuntu_app_launch_list_helper_instances(const gchar* type, const gchar* appid)
{
    ...
    helper_instances_t helper_instances_data = {g_strdup_printf("%s:", type), strlen(type) + 1, /* 1 for the colon */
                                                g_array_new(TRUE, TRUE, sizeof(gchar*)), g_strdup_printf(":%s", appid)};
...
static void helper_observer_cb(GDBusConnection* conn,
                               const gchar* sender,
                               const gchar* object,
                                   ...
                                   GList** list)
{
    ...
    *list = g_list_prepend(*list, observert);
    ...
    observert->sighandle = g_dbus_connection_signal_subscribe(conn, NULL,             /* sender */
...
static void set_var(GDBusConnection* bus, const gchar* job_name, const gchar* instance_name, const gchar* envvar)
{
    ...
    g_dbus_connection_call(bus, "com.ubuntu.Upstart", "/com/ubuntu/Upstart", "com.ubuntu.Upstart0_6", "SetEnv",

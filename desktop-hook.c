...
This is a hook for Click packages.  You can find information on Click package
hooks in
the click documentation:

https://click.readthedocs.org/en/latest/

Probably the biggest thing to understand for how this code works is that you
need to
understand that this hook is run after one, or many packages are installed.  A
set of
symbolic links are made to the desktop files per-application (not per-package)
in the
directory specified in ubuntu-app-launcher-desktop.click-hook.in.  Those desktop
files
give us the App ID of the packages that are installed and have applications
needing
desktop files in them.  We then operate on each of them ensuring that they are
synchronized
...
ubuntu-app-launch Upstart jobs.  They are there so that Unity can know which
applications
are installed for this user and they provide an Exec line to allow compatibility
with
desktop environments that are not using ubuntu-app-launch for launching
applications.
...
app_state_t *find_app_entry(const gchar *name, GArray *app_array) {
  int i;
  ...
  app_state_t *statepntr =
      &g_array_index(app_array, app_state_t, app_array->len - 1);
  return statepntr;
}

/* Looks up the file creation time, which seems harder with GLib
   than it should be */
guint64 modified_time(const gchar *dir, const gchar *filename) {
...
void add_click_package(const gchar *dir, const gchar *name, GArray *app_array) {
...
gboolean desktop_source_exists(const gchar *dir, const gchar *name) {
  gchar *desktopfile = g_build_filename(dir, name, NULL);

  GKeyFile *keyfile = g_key_file_new();
  g_key_file_load_from_file(keyfile, desktopfile, G_KEY_FILE_NONE,
                            NULL); /* No error */

  if (!g_key_file_has_key(keyfile, DESKTOP_GROUP, SOURCE_FILE_KEY, NULL)) {
    gboolean hasappid =
        g_key_file_has_key(keyfile, DESKTOP_GROUP, APP_ID_KEY, NULL);
    g_free(desktopfile);
  ...
  gchar *originalfile =
      g_key_file_get_string(keyfile, DESKTOP_GROUP, SOURCE_FILE_KEY, NULL);
  g_key_file_free(keyfile);
  gboolean found = TRUE;
...
void add_desktop_file(const gchar *dir, const gchar *name, GArray *app_array) {
  if (!g_str_has_suffix(name, ".desktop")) {
    return;
  }
...
void dir_for_each(const gchar *dirname,
                  void (*func)(const gchar *dir, const gchar *name,
                               GArray *app_array),
                  GArray *app_array) {
  GError *error = NULL;
...
/* Child watcher */
static gboolean apport_child_watch(GPid pid, gint status, gpointer user_data) {
...
static void report_recoverable_error(const gchar *app_id,
                                     const gchar *iconfield,
                                     const gchar *originalicon,
                                     const gchar *iconpath) {
  GError *error = NULL;
  gint error_stdin = 0;
  GPid pid = 0;
  gchar *argv[2] = {"/usr/share/apport/recoverable_problem", NULL};
  ...
  if (pid != 0) {
    ...
    g_source_set_callback(child_source, (GSourceFunc)apport_child_watch, loop,
                          NULL);
...
static void copy_desktop_file(const gchar *from, const gchar *to,
                              const gchar *appdir, const gchar *app_id) {
  GError *error = NULL;
  GKeyFile *keyfile = g_key_file_new();
  g_key_file_load_from_file(
      keyfile, from, G_KEY_FILE_KEEP_COMMENTS | G_KEY_FILE_KEEP_TRANSLATIONS,
      &error);

  if (error != NULL) {
    g_warning(
        "Unable to read the desktop file '%s' in the application directory: %s",
        from, error->message);
    g_error_free(error);
  ...
  if (g_key_file_has_key(keyfile, DESKTOP_GROUP, PATH_KEY, NULL)) {
    gchar *oldpath =
        g_key_file_get_string(keyfile, DESKTOP_GROUP, PATH_KEY, NULL);
    g_debug(
        "Desktop file '%s' has a Path set to '%s'.  Setting as " OLD_KEY_PREFIX
            PATH_KEY ".",
        from, oldpath);

    g_key_file_set_string(keyfile, DESKTOP_GROUP, OLD_KEY_PREFIX PATH_KEY,
                          oldpath);
  ...
  if (g_key_file_has_key(keyfile, DESKTOP_GROUP, ICON_KEY, NULL)) {
    gchar *originalicon =
        g_key_file_get_string(keyfile, DESKTOP_GROUP, ICON_KEY, NULL);
    ...
    if (g_file_test(iconpath, G_FILE_TEST_EXISTS)) {
      ...
      g_key_file_set_string(keyfile, DESKTOP_GROUP, OLD_KEY_PREFIX ICON_KEY,
                            originalicon);
    } else {
      /* So here we are, realizing all is lost.  Let's file a bug. */
      /* The goal here is to realize how often this case is, so we know how to
  ...
  if (g_key_file_has_key(keyfile, DESKTOP_GROUP, SYMBOLIC_ICON_KEY, NULL)) {
    gchar *originalicon =
        g_key_file_get_string(keyfile, DESKTOP_GROUP, SYMBOLIC_ICON_KEY, NULL);
    gchar *iconpath = g_build_filename(appdir, originalicon, NULL);

    /* If the icon in the path exists, let's use that */
    if (g_file_test(iconpath, G_FILE_TEST_EXISTS)) {
      g_key_file_set_string(keyfile, DESKTOP_GROUP, SYMBOLIC_ICON_KEY,
                            iconpath);
      /* Save the old value, because, debugging */
      g_key_file_set_string(keyfile, DESKTOP_GROUP,
                            OLD_KEY_PREFIX SYMBOLIC_ICON_KEY, originalicon);
    } else {
      /* So here we are, realizing all is lost.  Let's file a bug. */
      /* The goal here is to realize how often this case is, so we know how to
       * prioritize fixing it */

      report_recoverable_error(app_id, SYMBOLIC_ICON_KEY, originalicon,
                               iconpath);
    }
  ...
  gchar *newexec =
      g_strdup_printf("aa-exec-click -p %s -- %s", app_id, oldexec);
  ...
  if (error != NULL) {
    g_warning("Unable serialize keyfile built from '%s': %s", from,
              error->message);
    g_error_free(error);
    return;
  }
  ...
  if (error != NULL) {
    g_warning("Unable to write out desktop file to '%s': %s", to,
              error->message);
    g_error_free(error);
    return;
  }
...
static void build_desktop_file(app_state_t *state, const gchar *symlinkdir,
  ...
  ClickUser *user =
      click_user_new_for_user(db, g_getenv("TEST_CLICK_USER"), &error);
  g_object_unref(db);
  if (error != NULL) {
    g_warning("Unable to read Click database: %s", error->message);
    g_error_free(error);
  ...
  if (error != NULL) {
    g_warning("Unable to get the Click package directory for %s: %s", package,
              error->message);
    g_error_free(error);
    g_free(package);
    return;
...
static gboolean remove_desktop_file(app_state_t *state,
                                    const gchar *desktopdir) {
  gchar *desktopfile = g_strdup_printf("%s.desktop", state->app_id);
  gchar *desktoppath = g_build_filename(desktopdir, desktopfile, NULL);
  g_free(desktopfile);
  ...
  GKeyFile *keyfile = g_key_file_new();
  g_key_file_load_from_file(keyfile, desktoppath, G_KEY_FILE_NONE, NULL);
...
int main(int argc, char *argv[]) {
  ...
  gchar *symlinkdir = g_build_filename(g_get_user_cache_dir(),
                                       "ubuntu-app-launch", "desktop", NULL);

...
guint64 modified_time(const gchar* dir, const gchar* filename)
{
    ...
    GFileInfo* info =
        g_file_query_info(file, G_FILE_ATTRIBUTE_TIME_MODIFIED, G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS, NULL, NULL);
...
gboolean desktop_source_exists(const gchar* dir, const gchar* name)
{
    ...
    g_key_file_load_from_file(keyfile, desktopfile, G_KEY_FILE_NONE, NULL); /* No error */
...
void dir_for_each(const gchar* dirname,
                  void (*func)(const gchar* dir, const gchar* name, GArray* app_array),
                  GArray* app_array)
{
    GError* error = NULL;
...
{
    const char* properties[9] = {"IconValue", NULL, "AppID", NULL, "IconPath", NULL, "IconField", NULL, NULL};
...
static void copy_desktop_file(const gchar* from, const gchar* to, const gchar* appdir, const gchar* app_id)
{
    GError* error = NULL;
    GKeyFile* keyfile = g_key_file_new();
    g_key_file_load_from_file(keyfile, from, G_KEY_FILE_KEEP_COMMENTS | G_KEY_FILE_KEEP_TRANSLATIONS, &error);
...
static void build_desktop_file(app_state_t* state, const gchar* symlinkdir, const gchar* desktopdir)
{
    GError* error = NULL;
    gchar* package = NULL;
    /* 'Parse' the App ID */
...
static gboolean remove_desktop_file(app_state_t* state, const gchar* desktopdir)
{
    gchar* desktopfile = g_strdup_printf("%s.desktop", state->app_id);
    gchar* desktoppath = g_build_filename(desktopdir, desktopfile, NULL);
    g_free(desktopfile);

...
int main(int argc, char* argv[]) {
  ...
  if (appdir != NULL && strchr(appdir, ':') == NULL) {
    ...
    if (import_path != NULL && import_path[0] == '\0') import_path = NULL;
  ...
  if (g_getenv("MIR_SOCKET") != NULL &&
      g_strcmp0(g_getenv("APP_XMIR_ENABLE"), "1") == 0) {
  ...
  if (execret != 0) {
    ...
    g_warning("Unable to exec '%s' in '%s': %s", execprint, appdir,
              strerror(errno));
    g_free(execprint);
  }

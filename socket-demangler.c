...
int main(int argc, char* argv[]) {
  ...
  retval = g_dbus_connection_call_with_unix_fd_list_sync(
      bus, mir_name, mir_socket,
      "com.canonical.UbuntuAppLaunch.SocketDemangler", "GetMirSocket", NULL,
      G_VARIANT_TYPE("(h)"), G_DBUS_CALL_FLAGS_NO_AUTO_START, -1, /* timeout */
      NULL,          /* fd list in */
      &fdlist, NULL, /* cancelable */
      &error);
  ...
  if (handle >= g_unix_fd_list_get_length(fdlist)) {
    g_error("Handle is %d but the FD list only has %d entries", handle,

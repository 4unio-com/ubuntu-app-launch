...
static gboolean watchdog_timeout(gpointer user_data) {
  g_warning(
      "Watchdog triggered, took too long to submit into Zeitgeist Database!");
  g_main_loop_quit((GMainLoop *)user_data);

  return G_SOURCE_REMOVE;
}

static void insert_complete(GObject *obj, GAsyncResult *res,
                            gpointer user_data) {
  GError *error = NULL;
  GArray *result = NULL;
...
int main(int argc, char *argv[]) {
  if (argc != 2 ||
      (g_strcmp0(argv[1], "open") != 0 && g_strcmp0(argv[1], "close") != 0)) {

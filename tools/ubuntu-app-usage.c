...
GPtrArray *build_event_templates(void) {
...
gint sort_by_usage(gconstpointer a, gconstpointer b) {
...
void print_usage(GHashTable *usage) {
  ...
  while (g_hash_table_iter_next(&iter, &key, &value)) {
    ...
    usage_t usage = {.name = appurl, .seconds = GPOINTER_TO_UINT(value)};
...
void find_events_cb(GObject *obj, GAsyncResult *res, gpointer user_data) {
  ...
  GHashTable *laststop = g_hash_table_new_full(
      g_str_hash, g_str_equal, g_free, (GDestroyNotify)g_date_time_unref);
  GHashTable *usage =
  ...
  while (zeitgeist_result_set_has_next(results)) {
    ...
    if (g_strcmp0(zeitgeist_event_get_interpretation(event),
                  ZEITGEIST_ZG_ACCESS_EVENT) == 0) {
      eventtype = "started";
      GDateTime *stoptime = g_hash_table_lookup(laststop, appurl);
      if (stoptime != NULL) {
        GDateTime *starttime = g_date_time_new_from_unix_utc(
            zeitgeist_event_get_timestamp(event) / 1000);
        ...
        if (starttime != NULL) {
          ...
          gint previoususage =
              GPOINTER_TO_UINT(g_hash_table_lookup(usage, appurl));
          g_hash_table_insert(usage, g_strdup(appurl),
                              GUINT_TO_POINTER(previoususage + seconds));
        }
      ...
      GDateTime *stoptime = g_date_time_new_from_unix_utc(
          zeitgeist_event_get_timestamp(event) / 1000);
      if (stoptime != NULL) {
...
int main(int argc, char *argv[]) {
  ...
  zeitgeist_log_find_events(
      log, time,                              /* time range */
      templates, ZEITGEIST_STORAGE_STATE_ANY, /* storage state */
      10000,                                  /* num events */
      ZEITGEIST_RELEVANT_RESULT_TYPE_RECENT,  /* result type */
      NULL,                                   /* cancelable */
      find_events_cb, main_loop);

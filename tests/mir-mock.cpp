...
MirPromptSession *mir_connection_create_prompt_session_sync(
    MirConnection *connection, pid_t pid,
    void (*)(MirPromptSession *, MirPromptSessionState, void *data),
    void *context) {
  last_trust_pid = pid;
...
void mir_prompt_session_release_sync(MirPromptSession *session) {
...
MirWaitHandle *mir_prompt_session_new_fds_for_prompt_providers(
    MirPromptSession *session, unsigned int numfds, mir_client_fd_callback cb,
    void *data) {
  if (reinterpret_cast<char *>(session) != valid_trust_session) {
    std::cerr << "Releasing a Mir Trusted Prompt that isn't valid" << std::endl;
    exit(1);
  ...
  std::thread *thread = new std::thread([session, numfds, cb, data]() {
    ...
    for (unsigned int i = 0; i < numfds; i++) fdlist[i] = trusted_fd;
...
void mir_wait_for(MirWaitHandle *wait) {
...
void mir_mock_connect_return_valid(bool valid) { valid_connection = valid; }

std::pair<std::string, std::string> mir_mock_connect_last_connect(void) {
  return last_connection;
}

MirConnection *mir_connect_sync(char const *server, char const *appname) {
...
void mir_connection_release(MirConnection *con) {
...
void mir_mock_set_trusted_fd(int fd) { trusted_fd = fd; }

...
MirPromptSession* mir_connection_create_prompt_session_sync(
    MirConnection* connection, pid_t pid, void (*)(MirPromptSession*, MirPromptSessionState, void* data), void* context)
{
    last_trust_pid = pid;
...
MirWaitHandle* mir_prompt_session_new_fds_for_prompt_providers(
    ...
    unsigned int numfds,
    void (*cb)(MirPromptSession* session, size_t count, int const* fdin, void* user_data),
    void* data)
{
    if (reinterpret_cast<char*>(session) != valid_trust_session)
    {

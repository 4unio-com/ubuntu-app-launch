...
TEST_F(LibUAL, StartApplication) {
  ...
  auto appid =
      ubuntu::app_launch::AppID::parse("com.test.multiple_first_1.2.3");
  ...
  std::vector<ubuntu::app_launch::Application::URL> urls{
      ...
      ubuntu::app_launch::Application::URL::from_raw(
...
TEST_F(LibUAL, StopApplication) {
  ...
  auto appid =
      ubuntu::app_launch::AppID::parse("com.test.good_application_1.2.3");
  auto app = ubuntu::app_launch::Application::create(appid, registry);
...
TEST_F(LibUAL, ApplicationLog) {
  ...
  EXPECT_EQ(std::string(CMAKE_SOURCE_DIR "/libertine-data/upstart/"
                                         "application-click-com.test.good_"
                                         "application_1.2.3.log"),
  ...
  EXPECT_EQ(
      std::string(
          CMAKE_SOURCE_DIR
          "/libertine-data/upstart/application-legacy-multiple-2342345.log"),
      app->instances()[0]->logPath());
}
...
TEST_F(LibUAL, ApplicationPid) {
  ...
  auto singleapp =
      ubuntu::app_launch::Application::create(singleappid, registry);
  ...
  EXPECT_TRUE(g_variant_equal(
      calls->params,
      g_variant_new("(ss)", "freezer",
                    "upstart/application-legacy-multiple-2342345")));
  ASSERT_TRUE(
      dbus_test_dbus_mock_object_clear_method_calls(cgmock, cgobject, NULL));
}
...
TEST_F(LibUAL, ApplicationId) {
  ...
  EXPECT_EQ("com.test.good_application_1.2.3",
            (std::string)ubuntu::app_launch::AppID::discover("com.test.good",
                                                             "application"));
  ...
  EXPECT_EQ("com.test.good_application_1.2.4",
            (std::string)ubuntu::app_launch::AppID::discover(
                "com.test.good", "application", "1.2.4"));
  ...
  EXPECT_EQ(
      ...
      (std::string)ubuntu::app_launch::AppID::discover("com.test.multiple"));
  ...
  EXPECT_EQ("", (std::string)ubuntu::app_launch::AppID::discover(
                    "com.test.no-hooks"));
  EXPECT_EQ(
      "", (std::string)ubuntu::app_launch::AppID::discover("com.test.no-json"));
  EXPECT_EQ("", (std::string)ubuntu::app_launch::AppID::discover(
                    "com.test.no-object"));
  EXPECT_EQ("", (std::string)ubuntu::app_launch::AppID::discover(
                    "com.test.no-version"));
  ...
  /* Libertine tests */
  EXPECT_EQ("",
            (std::string)ubuntu::app_launch::AppID::discover("container-name"));
  EXPECT_EQ("", (std::string)ubuntu::app_launch::AppID::discover(
                    "container-name", "not-exist"));
...
TEST_F(LibUAL, AppIdParse) {
  EXPECT_FALSE(
      ubuntu::app_launch::AppID::parse("com.ubuntu.test_test_123").empty());
  EXPECT_FALSE(ubuntu::app_launch::AppID::find("inkscape").empty());
  EXPECT_FALSE(
      ubuntu::app_launch::AppID::parse("chatter.robert-ancell_chatter_2")
          .empty());
  EXPECT_FALSE(
      ubuntu::app_launch::AppID::find("chatter.robert-ancell_chatter").empty());
...
TEST_F(LibUAL, StartStopObserver) {
  ...
  dbus_test_dbus_mock_object_emit_signal(
      mock, obj, "EventEmitted", G_VARIANT_TYPE("(sas)"),
      g_variant_new_parsed("('started', ['JOB=application-legacy', "
                           "'INSTANCE=multiple-234235'])"),
      NULL);
...
TEST_F(LibUAL, LegacySingleInstance) {
  ...
  auto multipleapp =
      ubuntu::app_launch::Application::create(multipleappid, registry);
...
TEST_F(LibUAL, StartHelper) {
  ...
  guint len = 0;
  auto calls = dbus_test_dbus_mock_object_get_method_calls(mock, obj, "Start",
                                                           &len, NULL);
  EXPECT_NE(nullptr, calls);
  EXPECT_EQ(1, len);
  ...
  std::vector<ubuntu::app_launch::Helper::URL> urls = {
      ...
      ubuntu::app_launch::Helper::URL::from_raw(
          "file:///home/phablet/test.txt")};
  helper->launch(urls);
...
TEST_F(LibUAL, StopHelper) {
  ...
  guint len = 0;
  auto calls = dbus_test_dbus_mock_object_get_method_calls(mock, obj, "Stop",
                                                           &len, NULL);
  EXPECT_NE(nullptr, calls);
  EXPECT_EQ(1, len);
...
TEST_F(LibUAL, HelperList) {
  ...
  auto notlist =
      ubuntu::app_launch::Registry::runningHelpers(nothelper, registry);

  EXPECT_EQ(0, notlist.size());

  auto goodhelper =
      ubuntu::app_launch::Helper::Type::from_raw("untrusted-type");
  auto goodlist =
      ubuntu::app_launch::Registry::runningHelpers(goodhelper, registry);

  EXPECT_EQ(2, goodlist.size());

  goodlist.sort([](const std::shared_ptr<ubuntu::app_launch::Helper>& a,
                   const std::shared_ptr<ubuntu::app_launch::Helper>& b) {
    std::string sa = a->appId();
    std::string sb = b->appId();
...
TEST_F(LibUAL, AppInfo) {
  ...
  auto appid =
      ubuntu::app_launch::AppID::parse("com.test.good_application_1.2.4");
  auto app = ubuntu::app_launch::Application::create(appid, registry);
  ...
  EXPECT_THROW(ubuntu::app_launch::Application::create(barid, registry),
               std::runtime_error);

  /* Correct values for libertine */
  auto libertineid =
      ubuntu::app_launch::AppID::parse("container-name_test_0.0");
  auto libertine =
      ubuntu::app_launch::Application::create(libertineid, registry);

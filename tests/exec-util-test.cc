 ...
 protected:
  ...
  virtual void SetUp() {
    ...
    DbusTestDbusMockObject *jobobj = dbus_test_dbus_mock_get_object(
        mock, "/com/test/job", "com.ubuntu.Upstart0_6.Job", NULL);

    dbus_test_dbus_mock_object_add_method(
        mock, jobobj, "Start", G_VARIANT_TYPE("(asb)"), NULL, "", NULL);
  ...
  inline void StartCheckEnv(
      const std::string &appid,
      std::map<std::string, std::function<void(const gchar *)>> enums) {
    std::map<std::string, bool> env_found;
    DbusTestDbusMockObject *obj = dbus_test_dbus_mock_get_object(
        mock, "/com/test/job", "com.ubuntu.Upstart0_6.Job", nullptr);

    g_setenv("TEST_CLICK_DB", "click-db-dir", TRUE);
    g_setenv("TEST_CLICK_USER", "test-user", TRUE);
    g_setenv("UBUNTU_APP_LAUNCH_LINK_FARM", CMAKE_SOURCE_DIR "/link-farm",
             TRUE);
...
#define APP_DIR \
  CMAKE_SOURCE_DIR "/click-root-dir/.click/users/test-user/com.test.good"

  StartCheckEnv(
      "com.test.good_application_1.2.3",
      {
          {"UBUNTU_APPLICATION_ISOLATION",
           [](const gchar *value) { EXPECT_STREQ("1", value); }},
          ...
          {"APP_LAUNCHER_PID",
           [](const gchar *value) { EXPECT_EQ(getpid(), atoi(value)); }},
...
TEST_F(ExecUtil, DesktopExec) {
  StartCheckEnv(
      "foo",
      {
          ...
          {"APP_EXEC_POLICY",
           [](const gchar *value) { EXPECT_STREQ("unconfined", value); }},
...
TEST_F(ExecUtil, DesktopMir) {
  StartCheckEnv(
      "xmir",
      {
          {"APP_EXEC", [](const gchar *value) { EXPECT_STREQ("xfoo", value); }},
          {"APP_DESKTOP_FILE_PATH",
           [](const gchar *value) {
...
TEST_F(ExecUtil, DesktopNoMir) {
  StartCheckEnv(
      ...
      {
          {"APP_EXEC",
           [](const gchar *value) { EXPECT_STREQ("noxmir", value); }},
          {"APP_DESKTOP_FILE_PATH",
           [](const gchar *value) {
             EXPECT_STREQ(CMAKE_SOURCE_DIR "/applications/noxmir.desktop",
                          value);
           }},
          {"APP_EXEC_POLICY",
           [](const gchar *value) { EXPECT_STREQ("unconfined", value); }},
          ...
          {"APP_LAUNCHER_PID",
           [](const gchar *value) { EXPECT_EQ(getpid(), atoi(value)); }},
          {"APP_XMIR_ENABLE",
           [](const gchar *value) { EXPECT_STREQ("0", value); }},
      });
}
...
TEST_F(ExecUtil, LibertineExecUser) {
  StartCheckEnv(
      "container-name_user-app_0.0",
      {
          {"APP_EXEC",
           [](const gchar *value) {
             EXPECT_STREQ("libertine-launch \"container-name\" user-app",
                          value);
           }},
          {"APP_EXEC_POLICY",
           [](const gchar *value) { EXPECT_STREQ("unconfined", value); }},

...
TEST_F(FailureTest, CrashTest) {
  ...
  ASSERT_TRUE(ubuntu_app_launch_observer_add_app_failed(failed_observer,
                                                        &last_observer));

  /* Status based */
  ASSERT_TRUE(
      g_spawn_command_line_sync(APP_FAILED_TOOL, NULL, NULL, NULL, NULL));
  pause(100);
  ...
  ASSERT_TRUE(ubuntu_app_launch_observer_delete_app_failed(failed_observer,
                                                           &last_observer));
...
static void failed_start_observer(const gchar *appid,
                                  UbuntuAppLaunchAppFailed reason,
                                  gpointer user_data) {
...
TEST_F(FailureTest, StartTest) {
  ...
  std::string last_observer;
  ASSERT_TRUE(ubuntu_app_launch_observer_add_app_failed(failed_start_observer,
                                                        &last_observer));
  ...
  ASSERT_TRUE(ubuntu_app_launch_observer_delete_app_failed(
      failed_start_observer, &last_observer));

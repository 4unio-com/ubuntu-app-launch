...
public:
    GDBusMessage* FilterFunc(GDBusConnection* conn, GDBusMessage* message, gboolean incomming)
    {
        if (g_strcmp0(g_dbus_message_get_member(message), "UnityStartingBroadcast") == 0)
        {
            ...
            [body, correct_body]() { ASSERT_TRUE(g_variant_equal(body, correct_body)); }();
...
static GDBusMessage* filter_func(GDBusConnection* conn, GDBusMessage* message, gboolean incomming, gpointer user_data)
{
...
TEST_F(HelperHandshakeTest, BaseHandshake)
{
    ...
    g_dbus_connection_emit_signal(con, g_dbus_connection_get_unique_name(con), /* destination */
                                  "/",                                         /* path */

...
TEST_F(LibUAL, ApplicationLog)
{
    ...
    EXPECT_STREQ(CMAKE_SOURCE_DIR "/libertine-data/upstart/application-click-com.test.good_application_1.2.3.log",
                 click_log);
    g_free(click_log);
...
TEST_F(LibUAL, StartStopObserver)
{
    ...
    dbus_test_dbus_mock_object_emit_signal(
        mock, obj, "EventEmitted", G_VARIANT_TYPE("(sas)"),
        g_variant_new_parsed("('stopped', ['JOB=application-legacy', 'INSTANCE=multiple-9344321'])"), NULL);
...
TEST_F(LibUAL, DISABLED_PauseResume)
{
    ...
    dbus_test_dbus_mock_object_add_property(zgmock, zgobj, "version", G_VARIANT_TYPE("(iii)"),
                                            g_variant_new("(iii)", 0, 9, 16), nullptr);
    dbus_test_dbus_mock_object_add_property(zgmock, zgobj, "extensions", G_VARIANT_TYPE("as"),
                                            g_variant_new_array(G_VARIANT_TYPE_STRING, nullptr, 0), nullptr);
    dbus_test_dbus_mock_object_add_property(zgmock, zgobj, "datapath", G_VARIANT_TYPE("s"),
                                            g_variant_new_string(CMAKE_BINARY_DIR "/zg/activity.sqlite"), nullptr);
...
TEST_F(LibUAL, StartSessionHelper)
{
    ...
    /* Basic make sure we can send the event */
    gchar* instance_id =
        ubuntu_app_launch_start_session_helper("untrusted-type", msession, "com.test.multiple_first_1.2.3", NULL);
    ASSERT_NE(nullptr, instance_id);

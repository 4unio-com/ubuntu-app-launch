...
TEST(ZGEvent, OpenTest) {
  ...
  DbusTestDbusMockObject *obj =
      dbus_test_dbus_mock_get_object(mock, "/org/gnome/zeitgeist/log/activity",
                                     "org.gnome.zeitgeist.Log", NULL);

  dbus_test_dbus_mock_object_add_method(
      mock, obj, "InsertEvents", G_VARIANT_TYPE("a(asaasay)"),
  ...
  dbus_test_task_set_wait_for(DBUS_TEST_TASK(zgevent),
  ...
  ASSERT_EQ(dbus_test_task_get_state(DBUS_TEST_TASK(zgevent)),
            DBUS_TEST_TASK_STATE_FINISHED);
  ASSERT_TRUE(dbus_test_task_passed(DBUS_TEST_TASK(zgevent)));

  guint numcalls = 0;
  const DbusTestDbusMockCall *calls =
      dbus_test_dbus_mock_object_get_method_calls(mock, obj, "InsertEvents",
                                                  &numcalls, NULL);
...
static void zg_state_changed(DbusTestTask *task, DbusTestTaskState state,
                             gpointer user_data) {
  if (state != DBUS_TEST_TASK_STATE_FINISHED) return;
...
TEST(ZGEvent, TimeoutTest) {
  ...
  dbus_test_dbus_mock_object_add_method(mock, obj, "InsertEvents",
  ...
  dbus_test_task_set_wait_for(DBUS_TEST_TASK(zgevent),
                              "org.gnome.zeitgeist.Engine");
  dbus_test_task_set_name(DBUS_TEST_TASK(zgevent), "ZGEvent");
  g_signal_connect(G_OBJECT(zgevent), DBUS_TEST_TASK_SIGNAL_STATE_CHANGED,
                   G_CALLBACK(zg_state_changed), mainloop);

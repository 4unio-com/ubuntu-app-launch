...
int main(int argc, gchar* argv[]) {
  ...
  for (i = 0; appids[i] != NULL; i++) {
    gchar** instances =
        ubuntu_app_launch_list_helper_instances(type, appids[i]);
    ...
    if (instance_cnt == 0) {
      ...
      for (j = 0; j < instance_cnt; j++) {
        ...
        if (!ubuntu_app_launch_stop_multiple_helper(type, appids[i],
                                                    instances[j])) {
          g_warning("Unable to stop '%s' instance '%s'", appids[i],
                    instances[j]);
        }
      }
    }

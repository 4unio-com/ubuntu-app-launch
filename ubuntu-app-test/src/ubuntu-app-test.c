...
gboolean timeout(gpointer ploop) {
  g_main_loop_quit((GMainLoop *)ploop);
  return G_SOURCE_REMOVE;
}

void app_failed(const gchar *appid, UbuntuAppLaunchAppFailed failure_type,
                gpointer ploop) {
...
void fd_getter(MirPromptSession *session, size_t count, int const *fdsin,
               void *pfds) {
  if (count != 1) {
...
int main(int argc, char *argv[]) {
  ...
  gchar *mirpath =
      g_build_filename(g_get_user_runtime_dir(), "mir_socket_trusted", NULL);
  ...
  MirPromptSession *session =
      mir_connection_create_prompt_session_sync(mir, pid, NULL, NULL);

  int fd = 0;
  MirWaitHandle *wait = mir_prompt_session_new_fds_for_prompt_providers(
      session, 1, fd_getter, &fd);
